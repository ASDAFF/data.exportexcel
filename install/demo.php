...
function data_exportexcel_demo_expired() {
	$DemoMode = CModule::IncludeModuleEx("data.exportexcel");
	if ($DemoMode==MODULE_DEMO) {
		$now=time();
		if (defined("data_exportexcel_OLDSITEEXPIREDATE")) {
			if ($now>=data_exportexcel_OLDSITEEXPIREDATE || data_exportexcel_OLDSITEEXPIREDATE>$now+3000000) {
				return true;
			}
		} else{ 
			return true;
...
function data_exportexcel_show_demo($bAjax = false) {
	$moduleId = 'data.exportexcel';
	$DemoMode = CModule::IncludeModuleEx($moduleId);
	$activateText = GetMessage("DATA_EXPORTEXCEL_DEMO_MESSAGE_ACTIVATE_MODULE",array("#LANG#"=>LANGUAGE_ID));
	if ($DemoMode==MODULE_DEMO) {
		$now=time();
		if (defined("data_exportexcel_OLDSITEEXPIREDATE")) {
			if ($now<data_exportexcel_OLDSITEEXPIREDATE) {
				print BeginNote();
				$expire_arr = getdate(data_exportexcel_OLDSITEEXPIREDATE);
				...
				print GetMessage("DATA_EXPORTEXCEL_DEMO_MESSAGE_DAYS_REMAIN",array("#DAYS#"=>$days, "#ACTIVATE#"=>$activateText));
				print EndNote();
			} else {
				print BeginNote();
				print GetMessage("DATA_EXPORTEXCEL_DEMO_MESSAGE_EXPIRED", array("#ACTIVATE#"=>$activateText));
				print EndNote();
			}
		} else{ 
			print BeginNote();
	...
	else
	{
		if(!$bAjax)
		{
			print BeginNote('id="data-ee-updates-message"');
			print '<div id="data-ee-updates-message-inner"></div>';
			print EndNote();
		}
		else
		{
			...
			if($updateEnd && is_callable(array('CUpdateClientPartner', 'GetLicenseKey')))
			{
				...
				echo '<div id="data-ee-updates-message-inner">'.GetMessage("DATA_EXPORTEXCEL_DEMO_MESSAGE_UPDATE_END", array("#LCKEY#"=>$lckey, "#LANG#"=>LANGUAGE_ID)).'</div>';
			}
		}
	}
	?><script>BX.ready(function(){BX.bind(BX('data_module_activate'), 'click', function(){var div = BX.findNextSibling(this, {tag: 'DIV'}); BX.style(div, 'display', (BX.style(div, 'display')=='none' ? '' : 'none'));});});</script><?
}
