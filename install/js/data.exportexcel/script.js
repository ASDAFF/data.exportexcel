var dataIEModuleName = 'data.exportexcel';
var dataIEModuleFilePrefix = 'data_export_excel';
var dataIEModuleAddPath = '';
var dataIEModuleUMClass = 'data-ee-updates-message';
var EList = {
	Init: function()
	{
		if(!document.getElementById('data-ee-sheet-list')) return;
		...
		if(typeof anySelect.chosen != 'function')
		{
			...
			if(jQuerySrc)
			{
				$.getScript(jQuerySrc, function(){
					$.getScript('/bitrix/js/'+dataIEModuleName+'/chosen/chosen.jquery.min.js');
				});
			}
		}
		/*/Bug fix with excess jquery*/
		
		$('#data-ee-sheet-list .data-ee-sheet').each(function(){
		...
		$('#data-ee-sheet-list .data-ee-new-list-wrap input[type="button"]').bind('click', function(e){
	...
	InitLines: function(list)
	{
		...
		var sandwichSelector = '.data-ee-tbl .sandwich';
		if(typeof list!='undefined') sandwichSelector = '.data-ee-tbl[data-list-index='+list+'] .sandwich';
		
		var titlesSelector = '.data-ee-tbl tr.data-ee-tbl-titles';
		if(typeof list!='undefined') titlesSelector = '.data-ee-tbl[data-list-index='+list+'] tr.data-ee-tbl-titles';
		
		$(sandwichSelector).unbind('click').bind('click', function(){
			obj.sandwichOpened = this;
			var key = $(this).attr('data-key');
			var type = $(this).attr('data-type');
			var listIndex = $(this).closest('.data-ee-sheet').attr('data-sheet-index');
			
			var menuItems = [];
			if(key == 'COLUMN_TITLES')
			{
				menuItems.push({
					TEXT: BX.message("DATA_EE_CUSTOMIZE_FIELDS_LIST"),
					ONCLICK: "EList.ShowFieldsListSettings('"+listIndex+"')"
				});
				menuItems.push({
					TEXT: BX.message("DATA_EE_ADD_ALL_ELEMENT_FIELDS"),
					ONCLICK: "EList.AddAllFields('"+key+"', '"+listIndex+"', 'IE_')"
				});
				menuItems.push({
					TEXT: BX.message("DATA_EE_ADD_ALL_PROPERTIES"),
					ONCLICK: "EList.AddAllFields('"+key+"', '"+listIndex+"', 'IP_PROP')"
				});
				menuItems.push({
					TEXT: BX.message("DATA_EE_ADD_ALL_ELEMENT_FIELDS_SEO"),
					ONCLICK: "EList.AddAllFields('"+key+"', '"+listIndex+"', 'IPROP_TEMP_')"
				});
				menuItems.push({
					TEXT: BX.message("DATA_EE_ADD_ALL_ELEMENT_FIELDS_CATALOG"),
					ONCLICK: "EList.AddAllFields('"+key+"', '"+listIndex+"', 'ICAT_')"
				});
				menuItems.push({
					TEXT: BX.message("DATA_EE_ADD_LINE_ABOVE"),
					ONCLICK: "EList.AddNewRow('TEXT_ROWS_TOP', 0)"
				});
				menuItems.push({
					TEXT: BX.message("DATA_EE_ADD_LINE_UNDER"),
			...
			if(key.indexOf('TEXT_ROWS_TOP_') == 0 || key.indexOf('TEXT_ROWS_TOP2_') == 0)
			{
				...
				menuItems.push({
					TEXT: BX.message("DATA_EE_REMOVE_LINE"),
					ONCLICK: "EList.RemoveRow('"+key+"', '"+listIndex+"')"
				});
				menuItems.push({
					TEXT: BX.message("DATA_EE_INSERT_PICTURE"),
			...
			{
				menuItems.push({
					TEXT: BX.message("DATA_EE_INSERT_DATE"),
					ONCLICK: "EList.OpenAddTextBlock('DATE')"
				});
				menuItems.push({
					TEXT: BX.message("DATA_EE_INSERT_CURRENCY_RATE"),
					ONCLICK: "EList.OpenAddTextBlock('CURRENCY_RATE')"
				});
			}
			
			menuItems.push({
				TEXT: BX.message("DATA_EE_DISPLAY_SETTINGS"),
				ONCLICK: "EList.SetLineDisplaySetting('"+key+"', '"+listIndex+"', '"+type+"')"
			});
			menuItems.push({
				TEXT: BX.message("DATA_EE_DISPLAY_SETTINGS_RESET"),
		...
		/*$(titlesSelector).unbind('mouseover').bind('mouseover', function(){
			...
			var wrap = $(this).closest('.data-ee-sheet');
			wrap.append('<div class="data-ee-add-row-btn">+</div>');
			$('.data-ee-add-row-btn', wrap).css({top: top1-26});
		});
		$(titlesSelector).unbind('mouseout').bind('mouseout', function(){
			var wrap = $(this).closest('.data-ee-sheet');
			wrap.find('.data-ee-add-row-btn').remove();
		});*/
	},
	...
	AddNewRow: function(textKey, under)
	{
		...
		var wrap = tr.closest('.data-ee-sheet');
		...
		var newLine = '<tr style="display: none;"><td></td><td colspan="'+(tdCount-1)+'"><textarea class="data-ee-text-block" name="SETTINGS['+textKey+']['+listIndex+']['+maxKey+']"></textarea></td></tr>';
	...
	{
		//if(!gParent) gParent = $('.data-ie-tbl');
		var sheetParent = gParent.closest('.data-ee-sheet');
		$('.data-ee-hidden-settings select[name^="FIELDS_LIST["]', sheetParent).each(function(){
			var pSelect = this;
			var parent = $('tr.data-ee-tbl-titles', gParent);
	...
	OnFieldFocus: function(objInput)
	{
		...
		$(objInput).unbind('focus').bind('focus', function(){
			...
			var sheetParent = parentTbl.closest('.data-ee-sheet');
			var showCodes = (sheetParent.attr('data-show-field-codes') == 1);
			var pSelect = $('.data-ee-hidden-settings select[name^="FIELDS_LIST["]', sheetParent);
			...
			var chosenId = 'data_select_chosen';
			$('#'+chosenId).remove();
	...
	{
		tbl.closest('.data-ee-sheet').each(function(){
			EList.UpdateSheet($(this));
		});
	},
	...
	{
		params = params || {};
		var scrollLeft = $('.data-ee-tbl-wrap', wrap).scrollLeft();
		if(scrollLeft)
		{
			var wrapWidth = $('.data-ee-tbl-wrap', wrap).width();
			var innerWidth = $('.data-ee-tbl-wrap .data-ee-tbl', wrap).width();
		...
		wrap.append('<div class="data-ee-sheet-preloader"></div>'+
		...
		$.ajax({
			...
			success: function(data, textStatus, jqXHR)
			{
				...
				$('select[name$="_FILTER_PERIOD"], select[name$="_FILTER_DIRECTION"]', findForm).each(function(){
					...
					{
						$(this).append('<option value="last_days">'+BX.message("DATA_EE_FILTER_LAST_DAYS")+'</option>');
				...
				$('select[multiple]', findForm).each(function(){
					...
					if(parentDiv.find('.data-ee-select-view-mode').length == 0)
					{
						parentDiv.prepend('<a href="javascript:void(0)" onclick="EList.ChangeSelectViewMode(this)" class="data-ee-select-view-mode" title="'+BX.message("DATA_EE_SELECT_FAST_VIEW")+'"></a>');
					}
				});
				
				wrap.html(data);
				var ptable = $('.data-ee-tbl', wrap);
				EList.SetFieldValues(ptable);
				$('.data-ee-sheet-preloader', wrap).remove();
				EList.SetWidthList();
				
				$('.data-ee-tbl-wrap', wrap).bind('scroll', function(){
					$('#data_select_chosen').remove();
					$(this).prev('.data-ee-tbl-scroll').scrollLeft($(this).scrollLeft());
				});
				$('.data-ee-tbl-scroll', wrap).bind('scroll', function(){
					$('#data_select_chosen').remove();
					$(this).next('.data-ee-tbl-wrap').scrollLeft($(this).scrollLeft());
				});
				EList.InitLines();
				...
				var parentWrap = wrap.closest('.data-ee-sheet-wrap');
				$('.data-ee-new-list-wrap', parentWrap).show();
				var btnUp = (parentWrap.prev('.data-ee-sheet-wrap').length > 0);
				var btnDown = (parentWrap.next('.data-ee-sheet-wrap').length > 0);
				var btnBoth = (btnUp && btnDown);
				if(btnUp) $('.data-ee-title', wrap).append('<a href="javascript:void(0)" onclick="EList.MoveList(this, -1)" class="data-ee-list-up '+(!btnBoth ? 'data-ee-list-single' : '')+'" title="'+BX.message("DATA_EE_MOVE_LIST_UP")+'"></a>');
				if(btnDown) $('.data-ee-title', wrap).append('<a href="javascript:void(0)" onclick="EList.MoveList(this, 1)" class="data-ee-list-down '+(!btnBoth ? 'data-ee-list-single' : '')+'" title="'+BX.message("DATA_EE_MOVE_LIST_DOWN")+'"></a>');
				
				if(scrollLeft)
				{
					setTimeout(function(){
						$('.data-ee-tbl-wrap', wrap).scrollLeft(scrollLeft);
						$('.data-ee-tbl-scroll', wrap).scrollLeft(scrollLeft);
					}, 100);
				}
			},
		...
		$.post(window.location.href, post, function(data){
			...
			{
				$('.data-ee-tbl-wrap', wrap).scrollLeft(scrollLeft);
				setTimeout(function(){$('.data-ee-tbl-scroll', wrap).scrollLeft(scrollLeft);}, 100);
			}
		});*/
	},
	...
	ChangeSelectViewMode: function(a)
	{
		...
		if(select.length > 0 && typeof select.chosen == 'function')
		{
			if($(a).attr('mode')!='chosen')
			{
				...
				$(a).attr('title', BX.message("DATA_EE_SELECT_STANDARD_VIEW"));
				$(a).attr('mode', 'chosen');
			}
			else
			{
				select.chosen('destroy');
				$(a).attr('title', BX.message("DATA_EE_SELECT_FAST_VIEW"));
				$(a).attr('mode', '');
			}
		}
	},
	...
	AddColumn: function(btn)
	{
		...
		var wrap = parent.closest('.data-ee-sheet');
	...
	ShowLineActions: function(input)
	{
		...
		var title = admDATAMessages.lineActions[action].title;
		if(action.indexOf('SET_SECTION_')==0)
		{
			var style = input.value;
			if(!style) return;
			var level = parseInt(action.substr(12));
			$(input).closest('.data-ie-tbl').find('td.line-settings').each(function(){
				var td = $(this);
	...
	{
		$('.data-ee-tbl-wrap').each(function(){
			var div = $(this);
			div.css('width', 0);
			div.prev('.data-ee-tbl-scroll').css('width', 0);
			var timer = setInterval(function(){
				var width = div.parent().width();
				if(width > 0)
				{
					div.css('width', width);
					div.prev('.data-ee-tbl-scroll').css('width', width).find('>div').css('width', div.find('>table').width());
					clearInterval(timer);
				}
			}, 100);
	...
	{
		var tr = $(btn).closest('.data-ie-tbl').find('tr.settings');
		if(tr.is(':visible'))
		{
			tr.hide();
	...
	{
		var tbl = $(btn).closest('.data-ie-tbl');
	...
	{
		var tbl = $(link).closest('.data-ie-tbl');
		var tbls = tbl.parent().find('.data-ie-tbl').not(tbl);
		...
		var ts = tbl.find('.data-ie-field-select');
		for(var i=0; i<tbls.length; i++)
		{
			var tss = $('.data-ie-field-select', tbls[i]);
			for(var j=0; j<ts.length; j++)
			{
				...
				if(c2 < c1)
				{
					...
					{
						$('.data-ie-add-load-field', tss[j]).trigger('click');
					}
				}
				else if(c2 > c1)
				{
	...
	OnAfterAddNewProperty: function(fieldName, propId, propName, iblockId)
	{
		...
		var ptable = $(field).closest('.data-ie-tbl');
		$.post(window.location.href, post, function(data){			
	...
	{
		var parent = $(link).closest('.data-ie-field-select-btns');
	...
	ShowFieldSettings: function(btn)
	{
		...
		var ptable = $(btn).closest('.data-ee-tbl');
		var form = $(btn).closest('form')[0];
		
		var dialogParams = {
			'title':BX.message("DATA_EE_SETTING_UPLOAD_FIELD") + (input2.val() ? ' "'+input2.val()+'"' : ''),
			'content_url':'/bitrix/admin/'+dataIEModuleFilePrefix+'_field_settings.php?field='+val+'&field_name='+name+'&IBLOCK_ID='+ptable.attr('data-iblock-id')+'&PROFILE_ID='+form.PROFILE_ID.value,
			'width':'900',
			'height':'400',
			'resizable':true
		};
	...
	ShowListSettings: function(btn)
	{
		...
		var dialog = new BX.CAdminDialog({
			'title':'',
			'content_url':'/bitrix/admin/'+dataIEModuleFilePrefix+'_list_settings.php',
			'content_post': post,
			'width':'900',
			'height':'400',
			'resizable':true});
		...
		BX.addCustomEvent(dialog, 'onWindowRegister', function(){
			...
			if(typeof $('select.data-chosen-multi').chosen == 'function')
			{
				$('select.data-chosen-multi').chosen();
			}
		});
	...
	{
		var wrap = $(el).closest('.data-ee-sheet-wrap').find('.data-ee-sheet');
	...
	ApplyFilter: function(el)
	{
		...
		//var wrap = $(el).closest('.data-ee-sheet');
		var wrap = $(el).closest('.data-ee-sheet-wrap').find('.data-ee-sheet');
		EList.UpdateSheet(wrap);
		return false;
	},
	...
	SetExtraParams: function(oid, returnJson)
	{
		...
		{
			BX.WindowManager.Get().Close();
			var wrap = btn.closest('.data-ee-sheet');
			EList.UpdateSheet(wrap);
		}
	},
	...
	{
		var table = $('.data-ee-sheet[data-sheet-index='+listIndex+']');
		...
		var dialogParams = {
			'title':BX.message("DATA_EE_FIELDS_LIST_SETTINGS_TITLE"),
			'content_url':'/bitrix/admin/'+dataIEModuleFilePrefix+'_fields_list_settings.php?list_index='+listIndex+'&PROFILE_ID='+form.PROFILE_ID.value,
			'content_post': params,
			'width':'900',
			'height':'400',
			'resizable':true
		...
		BX.addCustomEvent(dialog, 'onWindowRegister', function(){
			...
			var parent = $('.data-ee-fl-settings-select', this.DIV);
	...
	{
		var wrap = $('.data-ee-sheet[data-sheet-index='+listIndex+']');
		var setParent = $('.data-ee-hidden-settings', wrap);
		var parent = $('tr.data-ee-tbl-titles', wrap);
	...
	{
		var parent = $('.data-ee-sheet[data-sheet-index='+listIndex+'] .data-ee-hidden-settings');
		...
		var dialogParams = {
			'title':BX.message("DATA_EE_DISPLAY_SETTINGS_TITLE"),
			'content_url':'/bitrix/admin/'+dataIEModuleFilePrefix+'_display_settings.php?key='+key+'&list_index='+listIndex+'&type='+type+'&PROFILE_ID='+form.PROFILE_ID.value,
	...
	{
		var wrap = $('.data-ee-sheet[data-sheet-index='+listIndex+']');
		var parent = $('.data-ee-hidden-settings', wrap);
	...
	Sort: function(link)
	{
		var wrap = $(link).closest('.data-ee-sheet');
		var setParent = $('.data-ee-hidden-settings', wrap);
	...
	AddNewList: function(btn, event)
	{
		...
		var i = 0;
		while(document.getElementById('data-ee-sheet-'+i)){i++;}
		//form.append('<input type="hidden" name="SETTINGS[LIST_NAME]['+i+']" value="">');
		if(copySettings)
		{
			var parent = $(btn).closest('.data-ee-sheet-wrap');
		...
		{
			$(btn).closest('.data-ee-sheet-wrap').after('<input type="hidden" name="SETTINGS[LIST_NAME]['+i+']" value="">');
		}
		this.ReloadWithoutSave(form);
	},
	...
	{
		var wrap = $(link).closest('.data-ee-sheet-wrap');
		if(direction < 0) var wrap2 = wrap.prev('.data-ee-sheet-wrap');
		else var wrap2 = wrap.next('.data-ee-sheet-wrap');
		if(wrap2.length > 0)
		{
			var title1 = $('.data-ee-title', wrap);
			var inputTitle1 = $('input[name^="SETTINGS[LIST_NAME]"]', title1);
			var title2 = $('.data-ee-title', wrap2);
	...
	{
		$(link).closest('.data-ee-sheet-wrap').remove();
	},
	...
	ShowAddTextMenu: function(btn)
	{
		...
		var arLines = [];
		arLines.push({'TEXT':BX.message("DATA_EE_ADD_TEXT_DATE"), 'TITLE':BX.message("DATA_EE_ADD_TEXT_DATE"), 'ONCLICK':'EList.OpenAddTextBlock("DATE")'});
	...
	{
		var dialogParams = {
			'title':BX.message("DATA_EE_INSERT_PARAMS"),
			'content_url':'/bitrix/admin/'+dataIEModuleFilePrefix+'_insert_params.php?code='+code,
			'width':'700',
			'height':'300',
			'resizable':true
		};
		...
		dialog.SetButtons([
			...
			new BX.CWindowButton(
			{
				title: BX.message('DATA_EE_INSERT_BTN'),
				id: 'savebtn',
				name: 'savebtn',
				className: BX.browser.IsIE() && BX.browser.IsDoctype() && !BX.browser.IsIE10() ? '' : 'adm-btn-save',
				action: function () {
...
var EProfile = {
	Init: function()
	{
		...
		if(select.length > 0)
		{
			...
			if(document.getElementById('data-ee-file-extension'))
			{
				$('#data-ee-file-extension').bind('change', function(){
					var ext = this.value;
					var arPath = $('#data-ee-file-path').val().split('.');
					if(arPath.length > 1)
					{
			...
			$('select.adm-detail-iblock-list').bind('change', function(){
				$.post(window.location.href, {'MODE': 'AJAX', 'IBLOCK_ID': this.value, 'ACTION': 'GET_UID'}, function(data){
					...
					{
						$('.data-sku-block.heading').show();
					}
					else
					{
						$('.data-sku-block').hide();
						$('.data-sku-block.heading .data-head-more').removeClass('show');
					}
				});
			});
			...
			if(typeof $('select.data-chosen-multi').chosen == 'function')
			{
				$('select.data-chosen-multi').chosen({width: '300px'});
			}
	...
	{
		var dialog = new BX.CAdminDialog({
			'title':BX.message("DATA_EE_POPUP_CRON_TITLE"),
			'content_url':'/bitrix/admin/'+dataIEModuleFilePrefix+'_cron_settings.php',
			'width':'800',
			'height':'350',
			'resizable':true});
	...
	RemoveProccess: function(link, id)
	{
		...
		$.ajax({
			...
			success: function(data){
				var parent = $(link).closest('.data-proccess-item');
				if(parent.parent().find('.data-proccess-item').length <= 1)
				{
	...
	{
		var bShow = ($('#data-ee-file-extension').val()=='csv');
...
var DataEeBx24 = {
				...
				$.ajax({
					...
					success: function(data){
						...
						for(var i=0; i<data.result.length; i++)
						{
							...
							var item = $('<div class="data-ee-bx24-struct-item"><input type="checkbox" name="bx24_folder_chb" value="'+curValue+'" onclick="DataEeBx24.SetFolder(this)" '+(curValue==value ? 'checked' : '')+'><a href="javascript:void(0)" onclick="DataEeBx24.ShowNextLevel(this)" data-folder-id="'+curValue+'" data-folder-path="'+curValue+'"><span>'+data.result[i].NAME+'</span></a></div>');
	...
	SetFolder: function(chb)
	{
		...
		$('#'+this.pathInputId).val($(chb).closest('.data-ee-bx24-struct-item').find('a').attr('data-folder-path'));
	},
	...
	ShowNextLevel: function(link)
	{
		...
		var parentWrap = link.closest('.data-ee-bx24-struct-item');
		var parentDiv = $('>.data-ee-bx24-struct-list', parentWrap);
		if(parentDiv.length > 0)
		{
			parentDiv.toggleClass('data-ee-bx24-struct-list-hidden');
			return;
		}
		parentDiv = $('<div class="data-ee-bx24-struct-list"></div>');
		...
		$.ajax({
			...
			success: function(data){
				...
				for(var i=0; i<data.result.length; i++)
				{
					...
					item = $('<div class="data-ee-bx24-struct-item"><input type="checkbox" name="bx24_folder_chb" value="'+curValue+'" onclick="DataEeBx24.SetFolder(this)" '+(curValue==value ? 'checked' : '')+'><a href="javascript:void(0)" onclick="DataEeBx24.ShowNextLevel(this)" data-folder-id="'+curValue+'" data-folder-path="'+fullCurValue+'">'+data.result[i].NAME+'</a></div>');
...
var EProfileList = {
	...
	{
		var dialogParams = {
			'title':BX.message("DATA_EE_POPUP_RESTORE_PROFILES_TITLE"),
			'content_url':'/bitrix/admin/'+dataIEModuleFilePrefix+'_restore_profiles.php',
			'width':'700',
			'height':'300',
			'resizable':true
		};
	...
	RestoreDialogButtonsSet: function(fireEvents)
	{
		...
		dialog.SetButtons([
			...
			new BX.CWindowButton(
			{
				title: BX.message('DATA_EE_POPUP_RESTORE_PROFILES_SAVE_BTN'),
				id: 'savebtn',
				name: 'savebtn',
				className: BX.browser.IsIE() && BX.browser.IsDoctype() && !BX.browser.IsIE10() ? '' : 'adm-btn-save',
				action: function () {
					...
					$.ajax({
						url: '/bitrix/admin/'+dataIEModuleFilePrefix+'_restore_profiles.php',
						type: 'POST',
...
var EImport = {
	...
	GetStatus: function()
	{
		...
		$.ajax({
			type: "GET",
			url: '/upload/tmp/'+dataIEModuleName+'/'+dataIEModuleAddPath+this.pid+'.txt?hash='+(new Date()).getTime(),
			success: function(data){
				var finish = false;
				if(data && data.substr(0, 1)=='{' && data.substr(data.length-1)=='}')
				{
	...
	UpdateStatus: function(result, end)
	{
		...
		if(typeof result == 'object')
		{
			...
			if(end)
			{
				...
				$('#data_ee_ready_file').css('visibility', 'visible');
	...
	SendData: function()
	{
		...
		$.ajax({
			...
			error: function(){
				...
				var timeBlock = document.getElementById('data_ee_auto_continue_time');
				if(timeBlock)
				{
					timeBlock.innerHTML = '';
	...
	TimeoutOnAutoConinue: function()
	{
		...
		else
		{
			...
			if(time < 1)
			{
				//$('#data_ie_continue_link').trigger('click');
...
var ESettings = {
	...
	{
		var div = $(link).closest('td').find('.data-ie-settings-margin:eq(0)');
		if(!div.is(':visible'))
		{
			div.show();
		}
	...
	{
		var divs = $(link).closest('td').find('.data-ie-settings-margin');
		if(divs.length > 1)
		{
			$(link).closest('.data-ie-settings-margin').remove();
		}
		else
		{
	...
	{
		$('.data-ee-settings-conversion').each(function(){
			var parent = this;
	...
	{
		var prevDiv = $(link).prev('.data-ee-settings-conversion');
	...
	{
		var div = $(link).closest('.data-ee-settings-conversion');
		if($(link).closest('td').find('.data-ee-settings-conversion').length > 1)
		{
			div.remove();
		}
		else
	...
	ShowChooseVal: function(btn)
	{
		...
		this.focusField = field;
		var arLines = [];
		for(var k in admDATASettingMessages)
		{
			arLines.push({'TEXT':'<b>'+admDATASettingMessages[k].TITLE+'</b>', 'HTML':'<b>'+admDATASettingMessages[k].TITLE+'</b>', 'TITLE':'#'+k+'# - '+admDATASettingMessages[k].TITLE,'ONCLICK':'javascript:void(0)'});
			for(var k2 in admDATASettingMessages[k].FIELDS)
			{
				arLines.push({'TEXT':admDATASettingMessages[k].FIELDS[k2], 'TITLE':'#'+k2+'# - '+admDATASettingMessages[k].FIELDS[k2],'ONCLICK':'ESettings.SetUrlVar(\'#'+k2+'#\')'});
			}
		}
		BX.adminShowMenu(btn, arLines, '');
	},
	...
	{
		var div = $(link).next('.data-ie-settings-phpexpression');
	...
	AddDefaultProp: function(select)
	{
		...
		var tmpl = parent.prev('tr.data-ie-list-settings-defaults');
		var tr = tmpl.clone();
		tr.css('display', '');
...
var EHelper = {
	...
	{
		var dialog = new BX.CAdminDialog({
			'title':BX.message("DATA_EE_POPUP_HELP_TITLE"),
			'content_url':'/bitrix/admin/'+dataIEModuleFilePrefix+'_popup_help.php',
			'width':'900',
			'height':'450',
			'resizable':true});
			
		BX.addCustomEvent(dialog, 'onWindowRegister', function(){
			$('#data-ie-help-faq > li > a').bind('click', function(){
			...
			if(index > 0)
			{
				$('#data-ie-help-tabs .data-ie-tabs-heads a:eq('+parseInt(index)+')').trigger('click');
			}
		});
	...
	{
		var parent = $(link).closest('.data-ee-tabs');
		var heads = $('.data-ee-tabs-heads a', parent);
		var bodies = $('.data-ee-tabs-bodies > div', parent);
		var index = 0;
...
$(document).ready(function(){
	...
	if($('#'+dataIEModuleUMClass).length > 0)
	{
		$.post('/bitrix/admin/'+dataIEModuleFilePrefix+'.php?lang='+BX.message('LANGUAGE_ID'), 'MODE=AJAX&ACTION=SHOW_MODULE_MESSAGE', function(data){
			data = $(data);
			var inner = $('#'+dataIEModuleUMClass+'-inner', data);
			if(inner.length > 0 && inner.html().length > 0)
			{
				$('#'+dataIEModuleUMClass+'-inner').replaceWith(inner);
				$('#'+dataIEModuleUMClass).show();
			}
		});
	}
});