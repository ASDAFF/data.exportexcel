...
/**
 * DATAPHPExcel_Writer_Excel5
 *
 * @category   DATAPHPExcel
 * @package    DATAPHPExcel_Writer_Excel5
 * @copyright  Copyright (c) 2006 - 2013 DATAPHPExcel (http://www.codeplex.com/DATAPHPExcel)
 */
class DATAPHPExcel_Writer_Excel5 extends DATAPHPExcel_Writer_Abstract implements DATAPHPExcel_Writer_IWriter
{
	/**
	 * DATAPHPExcel object
	 *
	...
	/**
	 * Create a new DATAPHPExcel_Writer_Excel5
	 *
	...
	public function __construct(DATAPHPExcel $phpExcel) {
		...
		$this->_parser		= new DATAPHPExcel_Writer_Excel5_Parser();
	}
	...
	public function save($pFilename = null) {
		...
		$this->_writerWorkbook = new DATAPHPExcel_Writer_Excel5_Workbook($this->_phpExcel,
		...
		for ($i = 0; $i < $countSheets; ++$i) {
			$this->_writerWorksheets[$i] = new DATAPHPExcel_Writer_Excel5_Worksheet($this->_str_total, $this->_str_unique,
		...
		$OLE = new DATAPHPExcel_Shared_OLE_PPS_File(DATAPHPExcel_Shared_OLE::Asc2Ucs($workbookStreamName));
		...
		if(isset($this->_documentSummaryInformation) && !empty($this->_documentSummaryInformation)){
			$OLE_DocumentSummaryInformation = new DATAPHPExcel_Shared_OLE_PPS_File(DATAPHPExcel_Shared_OLE::Asc2Ucs(chr(5) . 'DocumentSummaryInformation'));
		...
		if(isset($this->_summaryInformation) && !empty($this->_summaryInformation)){
		  $OLE_SummaryInformation = new DATAPHPExcel_Shared_OLE_PPS_File(DATAPHPExcel_Shared_OLE::Asc2Ucs(chr(5) . 'SummaryInformation'));
		...
		$root = new DATAPHPExcel_Shared_OLE_PPS_Root(time(), time(), $arrRootData);
		// save the OLE file
	 ...
	 * @return DATAPHPExcel_Writer_Excel5
	 */
	...
	private function _buildWorksheetEschers()
	{
		...
		foreach ($this->_phpExcel->getAllsheets() as $sheet) {
			...
			$escher = new DATAPHPExcel_Shared_Escher();

			// dgContainer
			$dgContainer = new DATAPHPExcel_Shared_Escher_DgContainer();
			...
			// spgrContainer
			$spgrContainer = new DATAPHPExcel_Shared_Escher_DgContainer_SpgrContainer();
			$dgContainer->setSpgrContainer($spgrContainer);

			// add one shape which is the group shape
			$spContainer = new DATAPHPExcel_Shared_Escher_DgContainer_SpgrContainer_SpContainer();
			...
			foreach ($sheet->getDrawingCollection() as $drawing) {
				...
				$twoAnchor = DATAPHPExcel_Shared_Excel5::oneAnchor2twoAnchor($sheet, $coordinates, $offsetX, $offsetY, $width, $height);
			...
			if(!empty($filterRange)){
				$rangeBounds = DATAPHPExcel_Cell::rangeBoundaries($filterRange);
				...
				while($iInc <= $iNumColEnd){
					...
					$oDrawing  = new DATAPHPExcel_Worksheet_BaseDrawing();
					// get the coordinates of drawing
					$cDrawing   = DATAPHPExcel_Cell::stringFromColumnIndex($iInc - 1) . $rangeBounds[0][1];
					...
					// set coordinates and offsets, client anchor
					$endCoordinates = DATAPHPExcel_Cell::stringFromColumnIndex(DATAPHPExcel_Cell::stringFromColumnIndex($iInc - 1));
	...
	private function _buildWorkbookEscher()
	{
		...
		// dggContainer
		$dggContainer = new DATAPHPExcel_Shared_Escher_DggContainer();
		...
		// bstoreContainer
		$bstoreContainer = new DATAPHPExcel_Shared_Escher_DggContainer_BstoreContainer();
			...
			foreach ($sheet->getDrawingCollection() as $drawing) {
				if ($drawing instanceof DATAPHPExcel_Worksheet_Drawing) {
					...
					case 1: // GIF, not supported by BIFF8, we convert to PNG
						$blipType = DATAPHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
						ob_start();
					...
					case 2: // JPEG
						$blipType = DATAPHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG;
					...
					case 6: // Windows DIB (BMP), we convert to PNG
						$blipType = DATAPHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
						ob_start();
						imagepng(DATAPHPExcel_Shared_Drawing::imagecreatefrombmp($filename));
						$blipData = ob_get_contents();
						ob_end_clean();
						break;
					...
					$blip = new DATAPHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE_Blip();
					$blip->setData($blipData);

					$BSE = new DATAPHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE();
					$BSE->setBlipType($blipType);
					$BSE->setBlip($blip);

					$bstoreContainer->addBSE($BSE);

				} else if ($drawing instanceof DATAPHPExcel_Worksheet_MemoryDrawing) {

					switch ($drawing->getRenderingFunction()) {

					case DATAPHPExcel_Worksheet_MemoryDrawing::RENDERING_JPEG:
						$blipType = DATAPHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG;
						$renderingFunction = 'imagejpeg';
						break;

					case DATAPHPExcel_Worksheet_MemoryDrawing::RENDERING_GIF:
					case DATAPHPExcel_Worksheet_MemoryDrawing::RENDERING_PNG:
					case DATAPHPExcel_Worksheet_MemoryDrawing::RENDERING_DEFAULT:
						$blipType = DATAPHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
	...
	private function _writeSummaryInformation(){
		...
		if($this->_phpExcel->getProperties()->getCreated()){
			...
			$dataSection[] = array('summary'=> array('pack' => 'V', 'data' => 0x0C),
								   ...
								   'data'	=> array('data' => DATAPHPExcel_Shared_OLE::LocalDate2OLE($dataProp)));
			$dataSection_NumProps++;
		}
		//	Modified Date/Time
