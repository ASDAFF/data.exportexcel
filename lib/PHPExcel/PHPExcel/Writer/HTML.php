 ...
 * @package	DATAPHPExcel_Writer_HTML
...
/**
 * DATAPHPExcel_Writer_HTML
 *
 * @category   DATAPHPExcel
 * @package	DATAPHPExcel_Writer_HTML
 * @copyright  Copyright (c) 2006 - 2013 DATAPHPExcel (http://www.codeplex.com/DATAPHPExcel)
 */
class DATAPHPExcel_Writer_HTML extends DATAPHPExcel_Writer_Abstract implements DATAPHPExcel_Writer_IWriter {
	/**
	 * DATAPHPExcel object
	 *
	 * @var DATAPHPExcel
	...
	/**
	 * Create a new DATAPHPExcel_Writer_HTML
	 *
	...
	private function _mapVAlign($vAlign) {
		switch ($vAlign) {
			case DATAPHPExcel_Style_Alignment::VERTICAL_BOTTOM:		return 'bottom';
			case DATAPHPExcel_Style_Alignment::VERTICAL_TOP:		return 'top';
			case DATAPHPExcel_Style_Alignment::VERTICAL_CENTER:
			case DATAPHPExcel_Style_Alignment::VERTICAL_JUSTIFY:	return 'middle';
	...
	private function _mapHAlign($hAlign) {
		switch ($hAlign) {
			case DATAPHPExcel_Style_Alignment::HORIZONTAL_GENERAL:				return false;
			case DATAPHPExcel_Style_Alignment::HORIZONTAL_LEFT:					return 'left';
			case DATAPHPExcel_Style_Alignment::HORIZONTAL_RIGHT:				return 'right';
			case DATAPHPExcel_Style_Alignment::HORIZONTAL_CENTER:
			case DATAPHPExcel_Style_Alignment::HORIZONTAL_CENTER_CONTINUOUS:	return 'center';
			case DATAPHPExcel_Style_Alignment::HORIZONTAL_JUSTIFY:				return 'justify';
	...
	private function _mapBorderStyle($borderStyle) {
		switch ($borderStyle) {
			case DATAPHPExcel_Style_Border::BORDER_NONE:				return 'none';
			case DATAPHPExcel_Style_Border::BORDER_DASHDOT:				return '1px dashed';
			case DATAPHPExcel_Style_Border::BORDER_DASHDOTDOT:			return '1px dotted';
			case DATAPHPExcel_Style_Border::BORDER_DASHED:				return '1px dashed';
			case DATAPHPExcel_Style_Border::BORDER_DOTTED:				return '1px dotted';
			case DATAPHPExcel_Style_Border::BORDER_DOUBLE:				return '3px double';
			case DATAPHPExcel_Style_Border::BORDER_HAIR:				return '1px solid';
			case DATAPHPExcel_Style_Border::BORDER_MEDIUM:				return '2px solid';
			case DATAPHPExcel_Style_Border::BORDER_MEDIUMDASHDOT:		return '2px dashed';
			case DATAPHPExcel_Style_Border::BORDER_MEDIUMDASHDOTDOT:	return '2px dotted';
			case DATAPHPExcel_Style_Border::BORDER_MEDIUMDASHED:		return '2px dashed';
			case DATAPHPExcel_Style_Border::BORDER_SLANTDASHDOT:		return '2px dashed';
			case DATAPHPExcel_Style_Border::BORDER_THICK:				return '3px solid';
			case DATAPHPExcel_Style_Border::BORDER_THIN:				return '1px solid';
	 ...
	 * @return DATAPHPExcel_Writer_HTML
	 */
	...
	public function generateHTMLHeader($pIncludeStyles = false) {
		// DATAPHPExcel object known?
		if (is_null($this->_phpExcel)) {
			throw new DATAPHPExcel_Writer_Exception('Internal DATAPHPExcel object not set to an instance of an object.');
		}
		...
		$html .= '<!-- Generated by DATAPHPExcel - http://www.phpexcel.net -->' . PHP_EOL;
		$html .= '<html>' . PHP_EOL;
		$html .= '  <head>' . PHP_EOL;
	...
	public function generateSheetData() {
		...
		foreach ($sheets as $sheet) {
			...
			$dimension[0] = DATAPHPExcel_Cell::coordinateFromString($dimension[0]);
			$dimension[0][0] = DATAPHPExcel_Cell::columnIndexFromString($dimension[0][0]) - 1;
			$dimension[1] = DATAPHPExcel_Cell::coordinateFromString($dimension[1]);
			$dimension[1][0] = DATAPHPExcel_Cell::columnIndexFromString($dimension[1][0]) - 1;
	...
	private function _extendRowsForChartsAndImages(DATAPHPExcel_Worksheet $pSheet, $row) {
		$rowMax = $row;
		$colMax = 'A';
		if ($this->_includeCharts) {
			foreach ($pSheet->getChartCollection() as $chart) {
				if ($chart instanceof DATAPHPExcel_Chart) {
				    $chartCoordinates = $chart->getTopLeftPosition();
				    $chartTL = DATAPHPExcel_Cell::coordinateFromString($chartCoordinates['cell']);
					$chartCol = DATAPHPExcel_Cell::columnIndexFromString($chartTL[0]);
					if ($chartTL[1] > $rowMax) {
						$rowMax = $chartTL[1];
						if ($chartCol > DATAPHPExcel_Cell::columnIndexFromString($colMax)) {
							$colMax = $chartTL[0];
						}
					}
				}
		...
		foreach ($pSheet->getDrawingCollection() as $drawing) {
			if ($drawing instanceof DATAPHPExcel_Worksheet_Drawing) {
			    $imageTL = DATAPHPExcel_Cell::coordinateFromString($drawing->getCoordinates());
				$imageCol = DATAPHPExcel_Cell::columnIndexFromString($imageTL[0]);
				if ($imageTL[1] > $rowMax) {
					$rowMax = $imageTL[1];
					if ($imageCol > DATAPHPExcel_Cell::columnIndexFromString($colMax)) {
						$colMax = $imageTL[0];
					}
				}
			}
	 ...
	 *
	 * @param	DATAPHPExcel_Worksheet	$pSheet			DATAPHPExcel_Worksheet
	 * @param	string				$coordinates	Cell coordinates
	 * @return	string
	 * @throws	DATAPHPExcel_Writer_Exception
	 */
	private function _writeImageInCell(DATAPHPExcel_Worksheet $pSheet, $coordinates) {
		// Construct HTML
		$html = '';
	 ...
	 * @throws	DATAPHPExcel_Writer_Exception
	 */
	private function _writeChartInCell(DATAPHPExcel_Worksheet $pSheet, $coordinates) {
		// Construct HTML
		$html = '';
		...
		foreach ($pSheet->getChartCollection() as $chart) {
			if ($chart instanceof DATAPHPExcel_Chart) {
				...
				if ($chartCoordinates['cell'] == $coordinates) {
					$chartFileName = DATAPHPExcel_Shared_File::sys_get_temp_dir().'/'.uniqid().'.png';
	...
	public function buildCSS($generateSurroundingHTML = true) {
		...
		foreach ($sheets as $sheet) {
			...
			$highestColumnIndex = DATAPHPExcel_Cell::columnIndexFromString($sheet->getHighestColumn()) - 1;
			$column = -1;
			...
			foreach ($sheet->getColumnDimensions() as $columnDimension) {
				if (($width = DATAPHPExcel_Shared_Drawing::cellDimensionToPixels($columnDimension->getWidth(), $this->_defaultFont)) >= 0) {
					$width = DATAPHPExcel_Shared_Drawing::pixelsToPoints($width);
					$column = DATAPHPExcel_Cell::columnIndexFromString($columnDimension->getColumnIndex()) - 1;
			...
			if ($rowDimension->getRowHeight() == -1) {
				$pt_height = DATAPHPExcel_Shared_Font::getDefaultRowHeightByFont($this->_phpExcel->getDefaultStyle()->getFont());
			} else {
	 ...
	 * Create CSS style
	 *
	 * @param	DATAPHPExcel_Style		$pStyle			DATAPHPExcel_Style
	 * @return	array
	 */
	private function _createCSSStyle(DATAPHPExcel_Style $pStyle) {
		// Construct CSS
		$css = '';
	...
	/**
	 * Create CSS style (DATAPHPExcel_Style_Alignment)
	 *
	 * @param	DATAPHPExcel_Style_Alignment		$pStyle			DATAPHPExcel_Style_Alignment
	 * @return	array
	 */
	private function _createCSSStyleAlignment(DATAPHPExcel_Style_Alignment $pStyle) {
		// Construct CSS
		$css = array();
	...
	/**
	 * Create CSS style (DATAPHPExcel_Style_Font)
	 *
	 * @param	DATAPHPExcel_Style_Font		$pStyle			DATAPHPExcel_Style_Font
	 * @return	array
	 */
	private function _createCSSStyleFont(DATAPHPExcel_Style_Font $pStyle) {
		// Construct CSS
		$css = array();
		...
		if ($pStyle->getUnderline() != DATAPHPExcel_Style_Font::UNDERLINE_NONE && $pStyle->getStrikethrough()) {
			$css['text-decoration'] = 'underline line-through';
		} else if ($pStyle->getUnderline() != DATAPHPExcel_Style_Font::UNDERLINE_NONE) {
	...
	/**
	 * Create CSS style (DATAPHPExcel_Style_Borders)
	 *
	 * @param	DATAPHPExcel_Style_Borders		$pStyle			DATAPHPExcel_Style_Borders
	 * @return	array
	 */
	private function _createCSSStyleBorders(DATAPHPExcel_Style_Borders $pStyle) {
		// Construct CSS
		$css = array();
	...
	/**
	 * Create CSS style (DATAPHPExcel_Style_Border)
	 *
	 * @param	DATAPHPExcel_Style_Border		$pStyle			DATAPHPExcel_Style_Border
	 * @return	string
	 */
	private function _createCSSStyleBorder(DATAPHPExcel_Style_Border $pStyle) {
		// Create CSS
//		$css = $this->_mapBorderStyle($pStyle->getBorderStyle()) . ' #' . $pStyle->getColor()->getRGB();
	...
	/**
	 * Create CSS style (DATAPHPExcel_Style_Fill)
	 *
	 * @param	DATAPHPExcel_Style_Fill		$pStyle			DATAPHPExcel_Style_Fill
	 * @return	array
	 */
	private function _createCSSStyleFill(DATAPHPExcel_Style_Fill $pStyle) {
		// Construct HTML
		$css = array();

		// Create CSS
		$value = $pStyle->getFillType() == DATAPHPExcel_Style_Fill::FILL_NONE ?
	 ...
	 *
	 * @param	DATAPHPExcel_Worksheet	$pSheet		The worksheet for the table we are writing
	 * @return	string
	 * @throws	DATAPHPExcel_Writer_Exception
	 */
	private function _generateTableHeader($pSheet) {
		...
		// Write <col> elements
		$highestColumnIndex = DATAPHPExcel_Cell::columnIndexFromString($pSheet->getHighestColumn()) - 1;
		$i = -1;
	 ...
	 * @throws	DATAPHPExcel_Writer_Exception
	 */
	private function _generateRow(DATAPHPExcel_Worksheet $pSheet, $pValues = null, $pRow = 0) {
		if (is_array($pValues)) {
			// Construct HTML
			$html = '';
			...
			foreach ($pValues as $cell) {
				$coordinate = DATAPHPExcel_Cell::stringFromColumnIndex($colNum) . ($pRow + 1);
				...
				// DATAPHPExcel_Cell
				if ($cell instanceof DATAPHPExcel_Cell) {
					$cellData = '';
					...
					if ($cell->getValue() instanceof DATAPHPExcel_RichText) {
						...
						if ($this->_preCalculateFormulas) {
							$cellData = DATAPHPExcel_Style_NumberFormat::toFormattedString(
					...
					if (!$this->_useInlineCss) {
						...
						if ($sharedStyle->getAlignment()->getHorizontal() == DATAPHPExcel_Style_Alignment::HORIZONTAL_GENERAL
				...
				if (isset($this->_isBaseCell[$pSheet->getParent()->getIndex($pSheet)][$pRow + 1][$colNum])) {
					...
					$endCellCoord = DATAPHPExcel_Cell::stringFromColumnIndex($colNum + $colSpan - 1) . ($pRow + $rowSpan);
				...
				if ($writeCell) {
					...
					$html .= '			<td';
						if (!$this->_useInlineCss) {
							...
							//** Necessary redundant code for the sake of DATAPHPExcel_Writer_PDF **
	...
	private function _calculateSpans()
	{
		...
		foreach ($sheetIndexes as $sheetIndex) {
			...
			foreach ($sheet->getMergeCells() as $cells) {
				list($cells, ) = DATAPHPExcel_Cell::splitRange($cells);
				$first = $cells[0];
				$last  = $cells[1];

				list($fc, $fr) = DATAPHPExcel_Cell::coordinateFromString($first);
				$fc = DATAPHPExcel_Cell::columnIndexFromString($fc) - 1;

				list($lc, $lr) = DATAPHPExcel_Cell::coordinateFromString($last);
				$lc = DATAPHPExcel_Cell::columnIndexFromString($lc) - 1;
			...
			$countColumns = DATAPHPExcel_Cell::columnIndexFromString($sheet->getHighestColumn());
	...
	private function _setMargins(DATAPHPExcel_Worksheet $pSheet) {
		$htmlPage = '@page { ';
		$htmlBody = 'body { ';

		$left = DATAPHPExcel_Shared_String::FormatNumber($pSheet->getPageMargins()->getLeft()) . 'in; ';
		$htmlPage .= 'left-margin: ' . $left;
		$htmlBody .= 'left-margin: ' . $left;
		$right = DATAPHPExcel_Shared_String::FormatNumber($pSheet->getPageMargins()->getRight()) . 'in; ';
		$htmlPage .= 'right-margin: ' . $right;
		$htmlBody .= 'right-margin: ' . $right;
		$top = DATAPHPExcel_Shared_String::FormatNumber($pSheet->getPageMargins()->getTop()) . 'in; ';
		$htmlPage .= 'top-margin: ' . $top;
		$htmlBody .= 'top-margin: ' . $top;
		$bottom = DATAPHPExcel_Shared_String::FormatNumber($pSheet->getPageMargins()->getBottom()) . 'in; ';
