...
/**
 * DATAPHPExcel_Shared_Escher_DggContainer_BstoreContainer
 *
 * @category   DATAPHPExcel
 * @package    DATAPHPExcel_Writer_Excel5
 * @copyright  Copyright (c) 2006 - 2013 DATAPHPExcel (http://www.codeplex.com/DATAPHPExcel)
 */
class DATAPHPExcel_Writer_Excel5_Escher
{
	/**
	...
	public function close()
	{
		...
		case 'DATAPHPExcel_Shared_Escher':
			if ($dggContainer = $this->_object->getDggContainer()) {
				$writer = new DATAPHPExcel_Writer_Excel5_Escher($dggContainer);
				$this->_data = $writer->close();
			} else if ($dgContainer = $this->_object->getDgContainer()) {
				$writer = new DATAPHPExcel_Writer_Excel5_Escher($dgContainer);
				$this->_data = $writer->close();
		...
		case 'DATAPHPExcel_Shared_Escher_DggContainer':
			...
			$header = pack('vvV', $recVerInstance, $recType, cp_strlen($dggData));
			$innerData .= $header . $dggData;

			// write the bstoreContainer
			if ($bstoreContainer = $this->_object->getBstoreContainer()) {
				$writer = new DATAPHPExcel_Writer_Excel5_Escher($bstoreContainer);
		...
		case 'DATAPHPExcel_Shared_Escher_DggContainer_BstoreContainer':
			// this is a container record
			...
			if ($BSECollection = $this->_object->getBSECollection()) {
				foreach ($BSECollection as $BSE) {
					$writer = new DATAPHPExcel_Writer_Excel5_Escher($BSE);
					$innerData .= $writer->close();
				}
			}
		...
		case 'DATAPHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE':
			...
			if ($blip = $this->_object->getBlip()) {
				$writer = new DATAPHPExcel_Writer_Excel5_Escher($blip);
				$innerData .= $writer->close();
			}
		...
		case 'DATAPHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE_Blip':
		...
		case 'DATAPHPExcel_Shared_Escher_DgContainer':
			// this is a container record
			...
			if ($spgrContainer = $this->_object->getSpgrContainer()) {
				$writer = new DATAPHPExcel_Writer_Excel5_Escher($spgrContainer);
				$innerData .= $writer->close();
		...
		case 'DATAPHPExcel_Shared_Escher_DgContainer_SpgrContainer':
			// this is a container record
			...
			foreach ($this->_object->getChildren() as $spContainer) {
				$writer = new DATAPHPExcel_Writer_Excel5_Escher($spContainer);
				$spData = $writer->close();
				$innerData .= $spData;

				// save the shape offsets (where new shape records begin)
				$totalSize += cp_strlen($spData);
				$spOffsets[] = $totalSize;
		...
		case 'DATAPHPExcel_Shared_Escher_DgContainer_SpgrContainer_SpContainer':
			// initialize
			$data = '';
			...
			if ($this->_object->getStartCoordinates()) {
				...
				// start coordinates
				list($column, $row) = DATAPHPExcel_Cell::coordinateFromString($this->_object->getStartCoordinates());
				$c1 = DATAPHPExcel_Cell::columnIndexFromString($column) - 1;
				$r1 = $row - 1;
				...
				// end coordinates
				list($column, $row) = DATAPHPExcel_Cell::coordinateFromString($this->_object->getEndCoordinates());
				$c2 = DATAPHPExcel_Cell::columnIndexFromString($column) - 1;
				$r2 = $row - 1;
				...
				$length			= cp_strlen($clientAnchorData);
			...
			if (!$this->_object->getSpgr()) {
				...
				$length = cp_strlen($clientDataData);
