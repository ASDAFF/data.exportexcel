...
/**
 * DATAPHPExcel_Writer_Excel2007_Chart
 *
 * @category   DATAPHPExcel
 * @package    DATAPHPExcel_Writer_Excel2007
 * @copyright  Copyright (c) 2006 - 2013 DATAPHPExcel (http://www.codeplex.com/DATAPHPExcel)
 */
class DATAPHPExcel_Writer_Excel2007_Chart extends DATAPHPExcel_Writer_Excel2007_WriterPart
{
	/**
	 ...
	 */
	public function writeChart(DATAPHPExcel_Chart $pChart = null)
	{
		// Create XML writer
		$objWriter = null;
	 ...
	 * Write Chart Title
	 *
	 * @param	DATAPHPExcel_Chart_Title		$title
	 * @param 	DATAPHPExcel_Shared_XMLWriter 	$objWriter 		XML Writer
	 * @throws 	DATAPHPExcel_Writer_Exception
	 */
	private function _writeTitle(DATAPHPExcel_Chart_Title $title = null, $objWriter)
	{
		if (is_null($title)) {
			return;
		}
	 ...
	 * Write Chart Legend
	 *
	 * @param	DATAPHPExcel_Chart_Legend		$legend
	 * @param 	DATAPHPExcel_Shared_XMLWriter 	$objWriter 		XML Writer
	 * @throws 	DATAPHPExcel_Writer_Exception
	 */
	private function _writeLegend(DATAPHPExcel_Chart_Legend $legend = null, $objWriter)
	{
		if (is_null($legend)) {
			return;
		}
	 ...
	 *
	 * @param	DATAPHPExcel_Chart_PlotArea		$plotArea
	 * @param	DATAPHPExcel_Chart_Title		$xAxisLabel
	 * @param	DATAPHPExcel_Chart_Title		$yAxisLabel
	 * @param 	DATAPHPExcel_Shared_XMLWriter 	$objWriter 		XML Writer
	 * @throws 	DATAPHPExcel_Writer_Exception
	 */
	private function _writePlotArea(DATAPHPExcel_Chart_PlotArea $plotArea,
									DATAPHPExcel_Chart_Title $xAxisLabel = NULL,
									DATAPHPExcel_Chart_Title $yAxisLabel = NULL,
									$objWriter,
									DATAPHPExcel_Worksheet $pSheet)
	{
		if (is_null($plotArea)) {
			return;
		}
				...
				$objWriter->startElement('c:'.$chartType);
					...
					for($i = 0; $i < $groupCount; ++$i) {
						...
						if ($groupType == $chartType) {
							...
							if ($groupType === DATAPHPExcel_Chart_DataSeries::TYPE_RADARCHART) {
								$objWriter->startElement('c:radarStyle');
									$objWriter->writeAttribute('val', $plotStyle );
								$objWriter->endElement();
							} elseif ($groupType === DATAPHPExcel_Chart_DataSeries::TYPE_SCATTERCHART) {
					...
					if ($chartType === DATAPHPExcel_Chart_DataSeries::TYPE_LINECHART) {
						//	Line only, Line3D can't be smoothed

						$objWriter->startElement('c:smooth');
							$objWriter->writeAttribute('val', (integer) $plotGroup->getSmoothLine() );
						$objWriter->endElement();
					} elseif (($chartType === DATAPHPExcel_Chart_DataSeries::TYPE_BARCHART) ||
						($chartType === DATAPHPExcel_Chart_DataSeries::TYPE_BARCHART_3D)) {
						...
						if ($plotGroupingType == 'percentStacked' ||
					...
					} elseif ($chartType === DATAPHPExcel_Chart_DataSeries::TYPE_BUBBLECHART) {
							...
							$objWriter->startElement('c:showNegBubbles');
								$objWriter->writeAttribute('val', 0 );
							$objWriter->endElement();
					} elseif ($chartType === DATAPHPExcel_Chart_DataSeries::TYPE_STOCKCHART) {
...
//					} while ($id1 == $id2);
					...
					if (($chartType !== DATAPHPExcel_Chart_DataSeries::TYPE_PIECHART) &&
						($chartType !== DATAPHPExcel_Chart_DataSeries::TYPE_PIECHART_3D) &&
						($chartType !== DATAPHPExcel_Chart_DataSeries::TYPE_DONUTCHART)) {
						...
						if ($chartType === DATAPHPExcel_Chart_DataSeries::TYPE_DONUTCHART) {
			...
			if (($chartType !== DATAPHPExcel_Chart_DataSeries::TYPE_PIECHART) &&
				...
				if ($chartType === DATAPHPExcel_Chart_DataSeries::TYPE_BUBBLECHART) {
	 ...
	 *
	 * @param 	DATAPHPExcel_Shared_XMLWriter 	$objWriter 		XML Writer
	 * @param 	DATAPHPExcel_Chart_Layout		$chartLayout	Chart layout
	 * @throws 	DATAPHPExcel_Writer_Exception
	 */
	 ...
	 * @throws 	DATAPHPExcel_Writer_Exception
	 */
	private function _writeCatAx($objWriter, DATAPHPExcel_Chart_PlotArea $plotArea, $xAxisLabel, $groupType, $id1, $id2, $isMultiLevelSeries)
	{
...
//										$objWriter->writeAttribute('xml:space', 'preserve');
										$objWriter->writeRawData(DATAPHPExcel_Shared_String::ControlCharacterPHP2OOXML( $caption ));
									$objWriter->endElement();
	 ...
	 * @throws 	DATAPHPExcel_Writer_Exception
	 */
	private function _writeValAx($objWriter, DATAPHPExcel_Chart_PlotArea $plotArea, $yAxisLabel, $groupType, $id1, $id2, $isMultiLevelSeries)
	{
...
//										$objWriter->writeAttribute('xml:space', 'preserve');
					...
					if ($groupType !== DATAPHPExcel_Chart_DataSeries::TYPE_BUBBLECHART) {
	...
	private static function _getChartType($plotArea)
	{
		...
		if ($groupCount == 1) {
			...
			if (count($chartTypes) == 0) {
				throw new DATAPHPExcel_Writer_Exception('Chart is not yet implemented');
			}
		}
	 ...
	 *
	 * @param	DATAPHPExcel_Chart_DataSeries		$plotGroup
									...
									)
	{
		...
		if (($groupType == DATAPHPExcel_Chart_DataSeries::TYPE_BARCHART) ||
			($groupType == DATAPHPExcel_Chart_DataSeries::TYPE_BARCHART_3D)) {
		...
		if (($groupType !== DATAPHPExcel_Chart_DataSeries::TYPE_RADARCHART) &&
			($groupType !== DATAPHPExcel_Chart_DataSeries::TYPE_STOCKCHART)) {

			if ($groupType !== DATAPHPExcel_Chart_DataSeries::TYPE_LINECHART) {
				if (($groupType == DATAPHPExcel_Chart_DataSeries::TYPE_PIECHART) ||
					($groupType == DATAPHPExcel_Chart_DataSeries::TYPE_PIECHART_3D) ||
					($groupType == DATAPHPExcel_Chart_DataSeries::TYPE_DONUTCHART) ||
					($plotSeriesCount > 1)) {
		...
		foreach($plotSeriesOrder as $plotSeriesIdx => $plotSeriesRef) {
			$objWriter->startElement('c:ser');
				...
				if (($groupType === DATAPHPExcel_Chart_DataSeries::TYPE_BARCHART) ||
					($groupType === DATAPHPExcel_Chart_DataSeries::TYPE_BARCHART_3D) ||
					($groupType === DATAPHPExcel_Chart_DataSeries::TYPE_BUBBLECHART)) {
				...
				if ($plotSeriesCategory && ($plotSeriesCategory->getPointCount() > 0)) {
					...
					if (($groupType === DATAPHPExcel_Chart_DataSeries::TYPE_BUBBLECHART) ||
						($groupType === DATAPHPExcel_Chart_DataSeries::TYPE_SCATTERCHART)) {
	 ...
	 *
	 * @param	DATAPHPExcel_Chart_DataSeriesValues		$plotSeriesLabel
	 * @param 	DATAPHPExcel_Shared_XMLWriter 			$objWriter 			XML Writer
	 * @throws 	DATAPHPExcel_Writer_Exception
	 */
	 ...
	 *
	 * @param	DATAPHPExcel_Chart_DataSeriesValues		$plotSeriesValues
	 * @param 	DATAPHPExcel_Shared_XMLWriter 			$objWriter 			XML Writer
	 * @param	string								$groupType			Type of plot for dataseries
	 ...
	 * @throws 	DATAPHPExcel_Writer_Exception
	 */
	private function _writeBubbles($plotSeriesValues, $objWriter, DATAPHPExcel_Worksheet $pSheet)
	{
		if (is_null($plotSeriesValues)) {
			return;
		}
	 ...
	 * Write Layout
	 *
	 * @param	DATAPHPExcel_Chart_Layout		$layout
	 * @param 	DATAPHPExcel_Shared_XMLWriter 	$objWriter 		XML Writer
	 * @throws 	DATAPHPExcel_Writer_Exception
	 */
	private function _writeLayout(DATAPHPExcel_Chart_Layout $layout = NULL, $objWriter)
	{
