...
/**
 * DATAPHPExcel_Writer_Excel2007_ContentTypes
 *
 * @category   DATAPHPExcel
 * @package    DATAPHPExcel_Writer_Excel2007
 * @copyright  Copyright (c) 2006 - 2013 DATAPHPExcel (http://www.codeplex.com/DATAPHPExcel)
 */
class DATAPHPExcel_Writer_Excel2007_ContentTypes extends DATAPHPExcel_Writer_Excel2007_WriterPart
{
	/**
	 ...
	 */
	public function writeContentTypes(DATAPHPExcel $pDATAPHPExcel = null, $includeCharts = FALSE)
	{
		// Create XML writer
		$objWriter = null;
		...
		$objWriter->writeAttribute('xmlns', 'http://schemas.openxmlformats.org/package/2006/content-types');
			...
			$chart = 1;
			for ($i = 0; $i < $sheetCount; ++$i) {
				$drawings = $pDATAPHPExcel->getSheet($i)->getDrawingCollection();
				$drawingCount = count($drawings);
				$chartCount = ($includeCharts) ? $pDATAPHPExcel->getSheet($i)->getChartCount() : 0;
			...
			// Comments
			for ($i = 0; $i < $sheetCount; ++$i) {
				if (count($pDATAPHPExcel->getSheet($i)->getComments()) > 0) {
					$this->_writeOverrideContentType(
			...
			for ($i = 0; $i < $mediaCount; ++$i) {
				...
				if ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof DATAPHPExcel_Worksheet_Drawing) {
					$extension 	= strtolower($this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getExtension());
					$mimeType 	= $this->_getImageMimeType( $this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getPath() );
				} else if ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof DATAPHPExcel_Worksheet_MemoryDrawing) {
			...
			$sheetCount = $pDATAPHPExcel->getSheetCount();
			for ($i = 0; $i < $sheetCount; ++$i) {
				if (count($pDATAPHPExcel->getSheet()->getHeaderFooter()->getImages()) > 0) {
					foreach ($pDATAPHPExcel->getSheet()->getHeaderFooter()->getImages() as $image) {
	...
	{
		if (DATAPHPExcel_Shared_File::file_exists($pFile)) {
			$image = getimagesize($pFile);
			return image_type_to_mime_type($image[2]);
		} else {
			throw new DATAPHPExcel_Writer_Exception("File $pFile does not exist");
		}
	}
	 ...
	 * @throws 	DATAPHPExcel_Writer_Exception
	 */
	private function _writeDefaultContentType(DATAPHPExcel_Shared_XMLWriter $objWriter = null, $pPartname = '', $pContentType = '')
	{
	 ...
	 * @throws 	DATAPHPExcel_Writer_Exception
	 */
	private function _writeOverrideContentType(DATAPHPExcel_Shared_XMLWriter $objWriter = null, $pPartname = '', $pContentType = '')
	{
		if ($pPartname != '' && $pContentType != '') {
			// Write content type
