...
/**
 * DATAPHPExcel_Reader_SYLK
 *
 * @category   DATAPHPExcel
 * @package    DATAPHPExcel_Reader
 * @copyright  Copyright (c) 2006 - 2013 DATAPHPExcel (http://www.codeplex.com/DATAPHPExcel)
 */
class DATAPHPExcel_Reader_SYLK extends DATAPHPExcel_Reader_Abstract implements DATAPHPExcel_Reader_IReader
{
	/**
	 * Input encoding
	 *
	...
	/**
	 * Create a new DATAPHPExcel_Reader_SYLK
	 */
	...
	public function listWorksheetInfo($pFilename)
	{
		...
		while (($rowData = fgets($fileHandle)) !== FALSE) {
			...
			$rowData = DATAPHPExcel_Shared_String::SYLKtoUTF8($rowData);
	...
	public function loadIntoExisting($pFilename, DATAPHPExcel $objDATAPHPExcel)
	{
		...
		$objDATAPHPExcel->setActiveSheetIndex( $this->_sheetIndex );
		...
		while (($rowData = fgets($fileHandle)) !== FALSE) {
									...
									for ($i=0;$i<strlen($styleSettings);++$i) {
										switch ($styleSettings{$i}) {
											...
											case 'D' :	$formatArray['font']['bold'] = true;
														break;
											case 'T' :	$formatArray['borders']['top']['style'] = DATAPHPExcel_Style_Border::BORDER_THIN;
														break;
											case 'B' :	$formatArray['borders']['bottom']['style'] = DATAPHPExcel_Style_Border::BORDER_THIN;
														break;
											case 'L' :	$formatArray['borders']['left']['style'] = DATAPHPExcel_Style_Border::BORDER_THIN;
														break;
											case 'R' :	$formatArray['borders']['right']['style'] = DATAPHPExcel_Style_Border::BORDER_THIN;
														break;
										}
									}
									break;
			...
			//	Read cell value data
				...
				$columnLetter = DATAPHPExcel_Cell::stringFromColumnIndex($column-1);
				$cellData = DATAPHPExcel_Calculation::_unwrapResult($cellData);

				// Set cell value
				$objDATAPHPExcel->getActiveSheet()->getCell($columnLetter.$row)->setValue(($hasCalculatedValue) ? $cellDataFormula : $cellData);
				if ($hasCalculatedValue) {
					$cellData = DATAPHPExcel_Calculation::_unwrapResult($cellData);
					$objDATAPHPExcel->getActiveSheet()->getCell($columnLetter.$row)->setCalculatedValue($cellData);
				}
			//	Read cell formatting
									...
									for ($i=0;$i<strlen($styleSettings);++$i) {
										switch ($styleSettings{$i}) {
											...
											case 'D' :	$styleData['font']['bold'] = true;
														break;
											case 'T' :	$styleData['borders']['top']['style'] = DATAPHPExcel_Style_Border::BORDER_THIN;
														break;
											case 'B' :	$styleData['borders']['bottom']['style'] = DATAPHPExcel_Style_Border::BORDER_THIN;
														break;
											case 'L' :	$styleData['borders']['left']['style'] = DATAPHPExcel_Style_Border::BORDER_THIN;
														break;
											case 'R' :	$styleData['borders']['right']['style'] = DATAPHPExcel_Style_Border::BORDER_THIN;
														break;
										}
									}
									break;
				...
				if (($formatStyle > '') && ($column > '') && ($row > '')) {
					...
					if (isset($this->_formats[$formatStyle])) {
						$objDATAPHPExcel->getActiveSheet()->getStyle($columnLetter.$row)->applyFromArray($this->_formats[$formatStyle]);
					}
				}
				if ((!empty($styleData)) && ($column > '') && ($row > '')) {
					$columnLetter = DATAPHPExcel_Cell::stringFromColumnIndex($column-1);
					$objDATAPHPExcel->getActiveSheet()->getStyle($columnLetter.$row)->applyFromArray($styleData);
				}
				if ($columnWidth > '') {
					if ($startCol == $endCol) {
						$startCol = DATAPHPExcel_Cell::stringFromColumnIndex($startCol-1);
						$objDATAPHPExcel->getActiveSheet()->getColumnDimension($startCol)->setWidth($columnWidth);
					} else {
						$startCol = DATAPHPExcel_Cell::stringFromColumnIndex($startCol-1);
						$endCol = DATAPHPExcel_Cell::stringFromColumnIndex($endCol-1);
						$objDATAPHPExcel->getActiveSheet()->getColumnDimension($startCol)->setWidth($columnWidth);
						do {
							$objDATAPHPExcel->getActiveSheet()->getColumnDimension(++$startCol)->setWidth($columnWidth);
	 ...
	 * @return DATAPHPExcel_Reader_SYLK
	 */
