...
/**
 * DATAPHPExcel_Reader_Excel2003XML
 *
 * @category   DATAPHPExcel
 * @package    DATAPHPExcel_Reader
 * @copyright  Copyright (c) 2006 - 2013 DATAPHPExcel (http://www.codeplex.com/DATAPHPExcel)
 */
class DATAPHPExcel_Reader_Excel2003XML extends DATAPHPExcel_Reader_Abstract implements DATAPHPExcel_Reader_IReader
{
	/**
	 * Formats
	 *
	...
	/**
	 * Create a new DATAPHPExcel_Reader_Excel2003XML
	 */
...
//		echo 'Character Set is ',$this->_charSet,'<br />';
	...
	public function loadIntoExisting($pFilename, DATAPHPExcel $objDATAPHPExcel)
	{
		...
		$underlineStyles = array (
				DATAPHPExcel_Style_Font::UNDERLINE_NONE,
				DATAPHPExcel_Style_Font::UNDERLINE_DOUBLE,
				DATAPHPExcel_Style_Font::UNDERLINE_DOUBLEACCOUNTING,
				DATAPHPExcel_Style_Font::UNDERLINE_SINGLE,
				DATAPHPExcel_Style_Font::UNDERLINE_SINGLEACCOUNTING
			);
		$verticalAlignmentStyles = array (
				DATAPHPExcel_Style_Alignment::VERTICAL_BOTTOM,
				DATAPHPExcel_Style_Alignment::VERTICAL_TOP,
				DATAPHPExcel_Style_Alignment::VERTICAL_CENTER,
				DATAPHPExcel_Style_Alignment::VERTICAL_JUSTIFY
			);
		$horizontalAlignmentStyles = array (
				DATAPHPExcel_Style_Alignment::HORIZONTAL_GENERAL,
				DATAPHPExcel_Style_Alignment::HORIZONTAL_LEFT,
				DATAPHPExcel_Style_Alignment::HORIZONTAL_RIGHT,
				DATAPHPExcel_Style_Alignment::HORIZONTAL_CENTER,
				DATAPHPExcel_Style_Alignment::HORIZONTAL_CENTER_CONTINUOUS,
				DATAPHPExcel_Style_Alignment::HORIZONTAL_JUSTIFY
			);
		...
		if (isset($xml->CustomDocumentProperties)) {
			foreach($xml->CustomDocumentProperties[0] as $propertyName => $propertyValue) {
				...
				$propertyName = preg_replace_callback('/_x([0-9a-z]{4})_/','DATAPHPExcel_Reader_Excel2003XML::_hex2str',$propertyName);
				$propertyType = DATAPHPExcel_DocumentProperties::PROPERTY_TYPE_UNKNOWN;
				switch((string) $propertyAttributes) {
					case 'string' :
						$propertyType = DATAPHPExcel_DocumentProperties::PROPERTY_TYPE_STRING;
						$propertyValue = trim($propertyValue);
						break;
					case 'boolean' :
						$propertyType = DATAPHPExcel_DocumentProperties::PROPERTY_TYPE_BOOLEAN;
						$propertyValue = (bool) $propertyValue;
						break;
					case 'integer' :
						$propertyType = DATAPHPExcel_DocumentProperties::PROPERTY_TYPE_INTEGER;
						$propertyValue = intval($propertyValue);
						break;
					case 'float' :
						$propertyType = DATAPHPExcel_DocumentProperties::PROPERTY_TYPE_FLOAT;
						$propertyValue = floatval($propertyValue);
						break;
					case 'dateTime.tz' :
						$propertyType = DATAPHPExcel_DocumentProperties::PROPERTY_TYPE_DATE;
...
//									echo $borderStyleKey.' = '.$borderStyleValue.'<br />';
									switch ($borderStyleKey) {
										case 'LineStyle' :
												$thisBorder['style'] = DATAPHPExcel_Style_Border::BORDER_MEDIUM;
			...
			if (isset($worksheet->Table->Column)) {
				foreach($worksheet->Table->Column as $columnData) {
					...
					if (isset($columnData_ss['Index'])) {
						$columnID = DATAPHPExcel_Cell::stringFromColumnIndex($columnData_ss['Index']-1);
					}
					if (isset($columnData_ss['Width'])) {
						$columnWidth = $columnData_ss['Width'];
//						echo '<b>Setting column width for '.$columnID.' to '.$columnWidth.'</b><br />';
						$objDATAPHPExcel->getActiveSheet()->getColumnDimension($columnID)->setWidth($columnWidth / 5.4);
					}
					++$columnID;
				}
			}
...
//					echo '<b>Row '.$rowID.'</b><br />';
					...
					foreach($rowData->Cell as $cell) {
						...
						if (isset($cell_ss['Index'])) {
							$columnID = DATAPHPExcel_Cell::stringFromColumnIndex($cell_ss['Index']-1);
						}
						...
						if ((isset($cell_ss['MergeAcross'])) || (isset($cell_ss['MergeDown']))) {
							...
							if (isset($cell_ss['MergeAcross'])) {
								$columnTo = DATAPHPExcel_Cell::stringFromColumnIndex(DATAPHPExcel_Cell::columnIndexFromString($columnID) + $cell_ss['MergeAcross'] -1);
							}
							$rowTo = $rowID;
							...
							$objDATAPHPExcel->getActiveSheet()->mergeCells($cellRange);
						}
							...
							if (isset($cellData_ss['Type'])) {
								...
								switch ($cellDataType) {
									...
									*/
									...
									case 'DateTime' :
											$type = DATAPHPExcel_Cell_DataType::TYPE_NUMERIC;
											$cellValue = DATAPHPExcel_Shared_Date::PHPToExcel(strtotime($cellValue));
											break;
									case 'Error' :
...
//								echo 'FORMULA<br />';
								...
								$columnNumber = DATAPHPExcel_Cell::columnIndexFromString($columnID);
									...
									foreach($temp as &$value) {
										...
										if ($key = !$key) {
											...
											foreach($cellReferences as $cellReference) {
												...
												$A1CellReference = DATAPHPExcel_Cell::stringFromColumnIndex($columnReference-1).$rowReference;
...
//
							$objDATAPHPExcel->getActiveSheet()->getCell($columnID.$rowID)->setValueExplicit((($hasCalculatedValue) ? $cellDataFormula : $cellValue),$type);
							if ($hasCalculatedValue) {
//								echo 'Formula result is '.$cellValue.'<br />';
								$objDATAPHPExcel->getActiveSheet()->getCell($columnID.$rowID)->setCalculatedValue($cellValue);
							}
...
//							echo 'Annotation: ',$annotation,'<br />';
							$objDATAPHPExcel->getActiveSheet()->getComment( $columnID.$rowID )
...
//								echo '<br />';
								if (!$objDATAPHPExcel->getActiveSheet()->cellExists($columnID.$rowID)) {
									$objDATAPHPExcel->getActiveSheet()->getCell($columnID.$rowID)->setValue(NULL);
								}
								$objDATAPHPExcel->getActiveSheet()->getStyle($cellRange)->applyFromArray($this->_styles[$style]);
							}
						}
						++$columnID;
					}
...
//							echo '<b>Setting row height to '.$rowHeight.'</b><br />';
							$objDATAPHPExcel->getActiveSheet()->getRowDimension($rowID)->setRowHeight($rowHeight);
						}
					}
	...
	private static function _convertStringEncoding($string,$charset) {
		if ($charset != 'UTF-8') {
			return DATAPHPExcel_Shared_String::ConvertEncoding($string,'UTF-8',$charset);
		}
		return $string;
	}
