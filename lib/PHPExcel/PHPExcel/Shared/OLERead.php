...
class DATAPHPExcel_Shared_OLERead {
	private $data = '';
	...
	public function read($sFileName)
	{
		...
		if(!is_readable($sFileName)) {
			throw new DATAPHPExcel_Reader_Exception("Could not open " . $sFileName . " for reading! File does not exist, or it is not readable.");
		}
		...
		if ($this->data != self::IDENTIFIER_OLE) {
			throw new DATAPHPExcel_Reader_Exception('The filename ' . $sFileName . ' is not recognised as an OLE file');
		}
		...
		for ($i = 0; $i < $this->numBigBlockDepotBlocks; ++$i) {
			...
			$this->bigBlockChain .= cp_substr($this->data, $pos, 4*$bbs);
			$pos += 4*$bbs;
		}
		...
		while ($sbdBlock != -2) {
			...
			$this->smallBlockChain .= cp_substr($this->data, $pos, 4*$bbs);
			$pos += 4*$bbs;
	...
	public function getStream($stream)
	{
		...
		if ($this->props[$stream]['size'] < self::SMALL_BLOCK_THRESHOLD) {
			...
			while ($block != -2) {
				...
				$streamData .= cp_substr($rootdata, $pos, self::SMALL_BLOCK_SIZE);
			...
			while ($block != -2) {
				...
				$streamData .= cp_substr($this->data, $pos, self::BIG_BLOCK_SIZE);
	...
	private function _readData($bl)
	{
		...
		while ($block != -2)  {
			$pos = ($block + 1) * self::BIG_BLOCK_SIZE;
			$data .= cp_substr($this->data, $pos, self::BIG_BLOCK_SIZE);
			$block = self::_GetInt4d($this->bigBlockChain, $block*4);
		}
		return $data;
	 }
	...
	private function _readPropertySets() {
		...
		$entryLen = cp_strlen($this->entry);
		while ($offset < $entryLen) {
			// entry data (128 bytes)
			$d = cp_substr($this->entry, $offset, self::PROPERTY_STORAGE_BLOCK_SIZE);
			...
			$name = str_replace("\x00", "", cp_substr($d,0,$nameSize));
