...
/**
 * DATAPHPExcel_CachedObjectStorage_Memcache
 *
 * @category   DATAPHPExcel
 * @package    DATAPHPExcel_CachedObjectStorage
 * @copyright  Copyright (c) 2006 - 2013 DATAPHPExcel (http://www.codeplex.com/DATAPHPExcel)
 */
class DATAPHPExcel_CachedObjectStorage_Memcache extends DATAPHPExcel_CachedObjectStorage_CacheBase implements DATAPHPExcel_CachedObjectStorage_ICache {
		...
		if ($this->_currentCellIsDirty) {
			...
			if (!$this->_memcache->replace($this->_cachePrefix.$this->_currentObjectID.'.cache',$obj,NULL,$this->_cacheTime)) {
				if (!$this->_memcache->add($this->_cachePrefix.$this->_currentObjectID.'.cache',$obj,NULL,$this->_cacheTime)) {
					$this->__destruct();
					throw new DATAPHPExcel_Exception('Failed to store cell '.$this->_currentObjectID.' in MemCache');
				}
			}
	...
	public function isDataSet($pCoord) {
		...
		if (parent::isDataSet($pCoord)) {
			...
			if ($success === false) {
				...
				throw new DATAPHPExcel_Exception('Cell entry '.$pCoord.' no longer exists in MemCache');
			}
			return true;
		}
		return false;
		...
		foreach($cacheList as $cellID) {
			if ($cellID != $this->_currentObjectID) {
				...
				if ($obj === false) {
					...
					throw new DATAPHPExcel_Exception('Cell entry '.$cellID.' no longer exists in MemCache');
				}
				if (!$this->_memcache->add($newCachePrefix.$cellID.'.cache',$obj,NULL,$this->_cacheTime)) {
					$this->__destruct();
					throw new DATAPHPExcel_Exception('Failed to store cell '.$cellID.' in MemCache');
				}
			}
		}
	 ...
	 *
	 * @param	DATAPHPExcel_Worksheet	$parent		The worksheet for this cell collection
	 * @param	array of mixed		$arguments	Additional initialisation arguments
	 */
	public function __construct(DATAPHPExcel_Worksheet $parent, $arguments) {
		...
		if (is_null($this->_cachePrefix)) {
			...
			if (!$this->_memcache->addServer($memcacheServer, $memcachePort, false, 50, 5, 5, true, array($this, 'failureCallback'))) {
				throw new DATAPHPExcel_Exception('Could not connect to MemCache server at '.$memcacheServer.':'.$memcachePort);
			}
	...
	public function failureCallback($host, $port) {
		throw new DATAPHPExcel_Exception('memcache '.$host.':'.$port.' failed');
	}
