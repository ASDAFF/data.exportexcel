...
/**
 * DATAPHPExcel_CachedObjectStorage_DiscISAM
 *
 * @category   DATAPHPExcel
 * @package    DATAPHPExcel_CachedObjectStorage
 * @copyright  Copyright (c) 2006 - 2013 DATAPHPExcel (http://www.codeplex.com/DATAPHPExcel)
 */
class DATAPHPExcel_CachedObjectStorage_DiscISAM extends DATAPHPExcel_CachedObjectStorage_CacheBase implements DATAPHPExcel_CachedObjectStorage_ICache {
	...
	public function copyCellCollection(DATAPHPExcel_Worksheet $parent) {
		...
		$newFileName = $this->_cacheDirectory.'/DATAPHPExcel.'.$baseUnique.'.cache';
	...
	public function __construct(DATAPHPExcel_Worksheet $parent, $arguments) {
		$this->_cacheDirectory	= ((isset($arguments['dir'])) && ($arguments['dir'] !== NULL))
									? $arguments['dir']
									: DATAPHPExcel_Shared_File::sys_get_temp_dir();

		parent::__construct($parent);
		if (is_null($this->_fileHandle)) {
			$baseUnique = $this->_getUniqueID();
			$this->_fileName = $this->_cacheDirectory.'/DATAPHPExcel.'.$baseUnique.'.cache';
			$this->_fileHandle = fopen($this->_fileName,'a+');
		}
