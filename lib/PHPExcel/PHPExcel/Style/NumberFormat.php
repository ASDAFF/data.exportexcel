...
/**
 * DATAPHPExcel_Style_NumberFormat
 *
 * @category   DATAPHPExcel
 * @package	DATAPHPExcel_Style
 * @copyright  Copyright (c) 2006 - 2013 DATAPHPExcel (http://www.codeplex.com/DATAPHPExcel)
 */
class DATAPHPExcel_Style_NumberFormat extends DATAPHPExcel_Style_Supervisor implements DATAPHPExcel_IComparable
{
	/* Pre-defined formats */
	 ...
	 * @var string
	 */
	protected $_formatCode	=	DATAPHPExcel_Style_NumberFormat::FORMAT_GENERAL;
	...
	/**
	 * Create a new DATAPHPExcel_Style_NumberFormat
	 *
	 ...
	 *
	 * <code>
	 * $objDATAPHPExcel->getActiveSheet()->getStyle('B2')->getNumberFormat()->applyFromArray(
	 *		array(
	 *			'code' => DATAPHPExcel_Style_NumberFormat::FORMAT_CURRENCY_EUR_SIMPLE
	 *		)
	 * );
	 * </code>
	 *
	 ...
	 * @return DATAPHPExcel_Style_NumberFormat
	 */
	public function setFormatCode($pValue = DATAPHPExcel_Style_NumberFormat::FORMAT_GENERAL)
	{
		if ($pValue == '') {
			$pValue = DATAPHPExcel_Style_NumberFormat::FORMAT_GENERAL;
		}
		if ($this->_isSupervisor) {
	...
	private static function fillBuiltInFormatCodes()
	{
		...
		if (is_null(self::$_builtInFormats)) {
			...
			// General
			self::$_builtInFormats[0] = DATAPHPExcel_Style_NumberFormat::FORMAT_GENERAL;
	...
	private static function _formatAsDate(&$value, &$format)
	{
		...
		$dateObj = DATAPHPExcel_Shared_Date::ExcelToPHPObject($value);
	...
	private static function _formatAsFraction(&$value, &$format)
	{
		...
		$GCD = DATAPHPExcel_Calculation_MathTrig::GCD($decimalPart,$decimalDivisor);
	 ...
	 */
	public static function toFormattedString($value = '0', $format = DATAPHPExcel_Style_NumberFormat::FORMAT_GENERAL, $callBack = null)
	{
		...
		if (($format === DATAPHPExcel_Style_NumberFormat::FORMAT_GENERAL) || ($format === DATAPHPExcel_Style_NumberFormat::FORMAT_TEXT)) {
			return $value;
		}
					...
					if (preg_match($number_regex, $m, $matches)) {
						...
						if ($useThousands) {
							$value = number_format(
										$value
										, strlen($right)
										, DATAPHPExcel_Shared_String::getDecimalSeparator()
										, DATAPHPExcel_Shared_String::getThousandsSeparator()
									);
