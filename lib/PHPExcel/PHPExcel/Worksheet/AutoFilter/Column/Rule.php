...
/**
 * DATAPHPExcel_Worksheet_AutoFilter_Column_Rule
 *
 * @category	DATAPHPExcel
 * @package		DATAPHPExcel_Worksheet
 * @copyright	Copyright (c) 2006 - 2013 DATAPHPExcel (http://www.codeplex.com/DATAPHPExcel)
 */
class DATAPHPExcel_Worksheet_AutoFilter_Column_Rule
{
...
//	const AUTOFILTER_COLUMN_RULE_ALLDATESINQUARTER	= 'allDatesInQuarter';	//	<dynamicFilter type="Q2"/> for Quarter 2
	 ...
	 * Autofilter Column
	 *
	 * @var DATAPHPExcel_Worksheet_AutoFilter_Column
	 */
	...
	/**
	 * Create a new DATAPHPExcel_Worksheet_AutoFilter_Column_Rule
	 *
	 * @param DATAPHPExcel_Worksheet_AutoFilter_Column $pParent
	 */
	public function __construct(DATAPHPExcel_Worksheet_AutoFilter_Column $pParent = NULL)
	{
	...
	public function setRuleType($pRuleType = self::AUTOFILTER_RULETYPE_FILTER) {
		if (!in_array($pRuleType,self::$_ruleTypes)) {
			throw new DATAPHPExcel_Exception('Invalid rule type for column AutoFilter Rule.');
		}
	...
	public function setValue($pValue = '') {
		if (is_array($pValue)) {
			...
			if (count($pValue) == 0) {
				throw new DATAPHPExcel_Exception('Invalid rule value for column AutoFilter Rule.');
			}
	...
	public function setOperator($pOperator = self::AUTOFILTER_COLUMN_RULE_EQUAL) {
		...
		if ((!in_array($pOperator,self::$_operators)) &&
			...
			throw new DATAPHPExcel_Exception('Invalid operator for column AutoFilter Rule.');
		}
	 ...
	 *
	 * @param DATAPHPExcel_Worksheet_AutoFilter_Column
	 * @return DATAPHPExcel_Worksheet_AutoFilter_Column_Rule
	 */
	public function setParent(DATAPHPExcel_Worksheet_AutoFilter_Column $pParent = NULL) {
		$this->_parent = $pParent;
