 ...
 * @package		DATAPHPExcel_Chart_Renderer
...
require_once(DATAPHPExcel_Settings::getChartRendererPath().'/jpgraph.php');


/**
 * DATAPHPExcel_Chart_Renderer_jpgraph
 *
 * @category	DATAPHPExcel
 * @package		DATAPHPExcel_Chart_Renderer
 * @copyright	Copyright (c) 2006 - 2013 DATAPHPExcel (http://www.codeplex.com/DATAPHPExcel)
 */
class DATAPHPExcel_Chart_Renderer_jpgraph
{
	...
	private function _formatDataSetLabels($groupID, $datasetLabels, $labelCount, $rotation = '') {
		...
		foreach($datasetLabels as $i => $datasetLabel) {
			if (is_array($datasetLabel)) {
				...
				if (!is_null($datasetLabelFormatCode)) {
					$datasetLabels[$i] = DATAPHPExcel_Style_NumberFormat::toFormattedString($datasetLabel,$datasetLabelFormatCode);
				}
			}
			++$testCurrentIndex;
		}
	...
	/**
	 * Create a new DATAPHPExcel_Chart_Renderer_jpgraph
	 */
	public function __construct(DATAPHPExcel_Chart $chart)
	{
		$this->_graph	= null;
		$this->_chart	= $chart;
