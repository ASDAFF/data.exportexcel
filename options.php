...
if($USER->IsAdmin())
{
	...
	$aTabs = array(
		array("DIV" => "edit0", "TAB" => Loc::getMessage("DATA_EE_SETTINGS"), "ICON" => "", "TITLE" => Loc::getMessage("DATA_EE_SETTINGS_TITLE")),
		array("DIV" => "edit1", "TAB" => Loc::getMessage("MAIN_TAB_RIGHTS"), "ICON" => "form_settings", "TITLE" => Loc::getMessage("MAIN_TAB_TITLE_RIGHTS")),
	);
	$tabControl = new CAdminTabControl("dataExportexcelTabControl", $aTabs, true, true);
	...
	$tabControl->Begin();
	?>
	<form method="POST" action="<?echo $APPLICATION->GetCurPage()?>?lang=<?echo LANGUAGE_ID?>&mid_menu=1&mid=<?=$module_id?>" name="data_exportexacel_settings">
	...
	?>
	<tr class="heading">
		<td colspan="2"><? echo Loc::getMessage('DATA_EE_OPTIONS_COMMON_SETTINGS'); ?></td>
	</tr>
	<tr>
		<td width="50%"><? echo Loc::getMessage('DATA_EE_SET_MAX_EXECUTION_TIME'); ?></td>
		<td width="50%">
	...
	<tr id="MAX_EXECUTION_TIME" <?if(!$setMaxExecutionTime){echo 'style="display: none;"';}?>>
		<td width="50%"><? echo Loc::getMessage('DATA_EE_MAX_EXECUTION_TIME'); ?></td>
		<td width="50%">
	...
	<tr id="EXECUTION_DELAY" <?if(!$setMaxExecutionTime){echo 'style="display: none;"';}?>>
		<td width="50%"><? echo Loc::getMessage('DATA_EE_EXECUTION_DELAY'); ?></td>
		<td width="50%">
	...
	<tr>
		<td width="50%"><? echo Loc::getMessage('DATA_EE_AUTO_CONTINUE_IMPORT'); ?></td>
		<td width="50%">
...
{
	if (confirm('<? echo CUtil::JSEscape(Loc::getMessage("DATA_EE_OPTIONS_BTN_HINT_RESTORE_DEFAULT_WARNING")); ?>'))
	...
	<input type="submit" name="Update" value="<?echo Loc::getMessage("DATA_EE_OPTIONS_BTN_SAVE")?>">
	<input type="hidden" name="Update" value="Y">
	<input type="reset" name="reset" value="<?echo Loc::getMessage("DATA_EE_OPTIONS_BTN_RESET")?>">
	<input type="button" title="<?echo Loc::getMessage("DATA_EE_OPTIONS_BTN_HINT_RESTORE_DEFAULT")?>" onclick="RestoreDefaults();" value="<?echo Loc::getMessage("DATA_EE_OPTIONS_BTN_RESTORE_DEFAULT")?>">
	<?$tabControl->End();?>
	</form>
<?
}
