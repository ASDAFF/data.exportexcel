...
$oProfile = new CDATAExportProfile('highload');
...
if ($REQUEST_METHOD == "POST" && $MODE=='AJAX')
{
	...
	if($ACTION=='DELETE_PROFILE')
	{
		$APPLICATION->RestartBuffer();
		ob_end_clean();
		$fl = new CDATAExportProfile('highload');
...
if ($REQUEST_METHOD == "POST" && $STEP > 1 && check_bitrix_sessid())
{
	...
	if ($STEP > 1)
	{
		...
		if (strlen($strError) <= 0)
		{
			...
			if(!$SETTINGS_DEFAULT['HIGHLOADBLOCK_ID'])
			{
				$strError.= GetMessage("DATA_EE_NO_HIGHLOADBLOCK")."<br>";
			}
		}
	...
	if($ACTION=='SHOW_PREVIEW')
	{
		...
		$fl = new CDATAEEFieldList($SETTINGS_DEFAULT);
		$listName = (isset($SETTINGS['LIST_NAME'][$list]) ? $SETTINGS['LIST_NAME'][$list] : sprintf(GetMessage("DATA_EE_SHEET_NAME"), $list+1));
		...
		$ee = new CDATAExportExcelHighload($params, $EXTRASETTINGS, false, $PROFILE_ID);
		...
		echo '<th><span class="sandwich" data-key="COLUMN_TITLES" title="'.GetMessage("DATA_EE_ACTIONS_BTN").'"></span></th>';
	...
	if($ACTION == 'DO_EXPORT')
	{
		...
		session_write_close();
		$_SESSION = $sess;
		$arResult = CDATAExportExcelRunner::ExportHighloadblock($params, $EXTRASETTINGS, $stepparams, $PROFILE_ID);
		$APPLICATION->RestartBuffer();
		ob_end_clean();
	...
	{
		/*$params = array_merge($SETTINGS_DEFAULT, $SETTINGS);
		$ie = new CDATAExportExcelHighload($DATA_FILE_NAME, $params);
		$ie->Import();
		die();*/
	}
	//*****************************************************************//
...
<?$aTabs = array(
	...
	array(
		...
		"ICON" => "iblock",
		"TITLE" => GetMessage("DATA_EE_TAB2_ALT"),
	) ,
	array(
		"DIV" => "edit3",
		"TAB" => GetMessage("DATA_EE_TAB3") ,
		"ICON" => "iblock",
		"TITLE" => GetMessage("DATA_EE_TAB3_ALT"),
	) ,
);
...
?>
	...
	?>
		...
		<tr class="heading">
			<td colspan="2"><?echo GetMessage("DATA_EE_SETTINGS_DISPLAY"); ?></td>
		</tr>
...
?>
	...
	<tr>
		<td colspan="2" id="data-ee-sheet-list">
			<div class="data-ee-sheet-wrap">
				<?
					$list = 0;
					$filterId = 'data_exportexcel_highload_'.$PROFILE_ID.'_'.$list;
					CDATAExportUtils::ShowFilterHighload($filterId, $list, $SETTINGS, $SETTINGS_DEFAULT);
				?>
				<div class="data-ee-sheet" id="data-ee-sheet-0" data-sheet-index="0">
				</div>
			</div>
		</td>
	</tr>
		 ...
		 <table width="100%"><tr><td width="50%">
			...
			<div id="block_error_import" style="display: none;">
				<?echo CAdminMessage::ShowMessage(array(
					...
					"DETAILS" => '<div><a href="javascript:void(0)" onclick="EProfile.ContinueProccess(this, '.$PROFILE_ID.');">'.GetMessage("DATA_EE_PROCESSED_CONTINUE").'</a></div>',
					"HTML" => true,
				))?>
			</div>
			...
			<div class="detail_status">
				<?echo CAdminMessage::ShowMessage(array(
					...
					' <span id="data_ee_ready_file" style="visibility: hidden;"><br>'.GetMessage("DATA_EE_READY_FILE_LINK").' <br><a href="'.htmlspecialcharsex($SETTINGS_DEFAULT['FILE_PATH']).'?hash='.md5(mt_rand()).'">'.$SETTINGS_DEFAULT['FILE_PATH'].'</a></span><br>',
					"HTML" => true,
				))?>
			</div>
		 </td></tr></table>
