...
$moduleRunnerClass = 'CDATAExportExcelRunner';
...
$siteEncoding = CDATAExportUtils::getSiteEncoding();
...
if ($REQUEST_METHOD == "POST" && $MODE=='AJAX')
{
	...
	{
		CDATAExportUtils::RemoveTmpFiles();
		CDATAExportUtils::CheckZipArchive();
		die();
	}
	...
	{
		$fl = new CDATAEEFieldList($SETTINGS_DEFAULT);
		$APPLICATION->RestartBuffer();
		ob_end_clean();
		?><div><?
	...
	if($ACTION=='GET_UID')
	{
		$OFFERS_IBLOCK_ID = CDATAExportUtils::GetOfferIblock($IBLOCK_ID);
		ob_end_clean();
...
if ($REQUEST_METHOD == "POST" && $STEP > 1 && check_bitrix_sessid())
{
	...
	if ($STEP > 1)
	{
		...
		{
			$strError.= GetMessage("DATA_EE_PROFILE_NOT_CHOOSE")."<br>";
		}
		
		if (strlen($strError) <= 0)
		{
			if (!$SETTINGS_DEFAULT['IBLOCK_ID'])
				$strError.= GetMessage("DATA_EE_NO_IBLOCK")."<br>";
		}
		...
		if (strlen($strError) <= 0)
		{
			...
			if(strpos($fpath, '/')!==0)
				$strError.= GetMessage("DATA_EE_FILE_PATH_INCORRECT")."<br>";
		}
	...
	if($ACTION=='SHOW_PREVIEW')
	{
		...
		$listName = ($SETTINGS['LIST_NAME'][$list] ? $SETTINGS['LIST_NAME'][$list] : sprintf(GetMessage("DATA_EE_SHEET_NAME"), $list+1));
		
		/*$filterId = 'data_exportexcel_'.$PROFILE_ID.'_'.$list;
		CDATAExportUtils::ShowFilter($filterId, $list, $SETTINGS, $SETTINGS_DEFAULT);*/
		...
		$ee = new CDATAExportExcel($params, $EXTRASETTINGS, false, $PROFILE_ID);
		...
		foreach($textKeys as $textKey)
		{
			...
			if(!empty($SETTINGS[$textKey][$list]))
			{
				foreach($SETTINGS[$textKey][$list] as $k=>$v)
				{
					...
					$rowContent .= '<tr title="'.sprintf(GetMessage("DATA_EE_ROW_NUMBER"), $rowIndex++).'">';
					$rowContent .= '<td><span class="sandwich" data-key="'.$dataKey.'" data-type="'.$dataType.'" title="'.GetMessage("DATA_EE_ACTIONS_BTN").'"></span></td>';
					if($dataType == 'image')
					{
						$rowContent .= '<td colspan="'.count($arFields).'"><div class="cell cell_wide80"><div class="cell_inner" '.CDATAExportUtils::GetCellStyleFormatted($dSettings, $SETTINGS_DEFAULT).'>';
					...
					else
					{
						...
						$rowContent .= '<textarea class="data-ee-text-block" name="SETTINGS['.$textKey.']['.$list.']['.$k.']" '.CDATAExportUtils::GetCellStyleFormatted($dSettings, $SETTINGS_DEFAULT).'>'.$v.'</textarea>';
						//$rowContent .= '<input class="data-ee-text-block-val" value="..." onclick="EList.ShowAddTextMenu(this);" type="button" title="'.GetMessage("DATA_EE_ADD_TEXT_VAL").'">';
						$rowContent .= '</div></div></td>';
					}
					$rowContent .= '</tr>';
		...
		ob_end_clean();
		?>
		<div class="data-ee-title">
			<input type="text" name="SETTINGS[LIST_NAME][<?echo $list?>]" value="<?echo htmlspecialcharsbx($listName)?>" maxlength="31">
			<?if($list > 0){?>
				<a href="javascript:void(0)" class="data-ee-remove-list" onclick="EList.RemoveList(this);" title="<?echo GetMessage("DATA_EE_REMOVE_LIST"); ?>"></a>
			<?}?>
		</div>
		<div class="data-ee-hidden-settings">
		...
		<div class="data-ee-additional-settings">
			<a href="javascript:void(0)" class="addsettings_link" onclick="EList.ToggleAddSettingsBlock(this)"><span><?echo GetMessage("DATA_EE_ADDITIONAL_SETTINGS"); ?></span></a>
			<div class="addsettings_inner">
				<table class="additional">
					<col><col width="400px">
					<tr>
						<td><?echo GetMessage("DATA_EE_LIST_LABEL_COLOR"); ?>:</td>
						<td>
					...
					<tr>
						<td><?echo GetMessage("DATA_EE_HIDE_COLUMN_TITLES"); ?>:</td>
						<td>
					...
					<tr>
						<td><?echo GetMessage("DATA_EE_ENABLE_AUTOFILTER"); ?>:</td>
						<td>
					...
					<tr>
						<td><?echo GetMessage("DATA_EE_ENABLE_PROTECTION"); ?>:</td>
						<td>
					...
					<tr>
						<td><?echo GetMessage("DATA_EE_SHOW_ONLY_SECTION_PROPERTY"); ?>:</td>
						<td>
					...
					<tr>
						<td><?echo GetMessage("DATA_EE_SHOW_ONLY_SECTION_FROM_FILTER"); ?>:</td>
						<td>
					...
					<tr>
						<td><?echo GetMessage("DATA_EE_CHANGE_IBLOCK_ID"); ?>:</td>
						<td>
					...
					<tr class="subfield" <?if(!$changeIblockId){echo 'style="display: none;"';}?>>
						<td><?echo GetMessage("DATA_EE_INFOBLOCK"); ?>:</td>
						<td>
		...
		<?		
		echo '<div class="data-ee-tbl-scroll"><div></div></div>';
		echo '<div class="data-ee-tbl-wrap">';
		echo '<table class="data-ee-tbl" data-iblock-id="'.$iblockId.'">';
		...
		echo '<tr class="data-ee-tbl-titles">';
		echo '<th title="'.sprintf(GetMessage("DATA_EE_ROW_NUMBER"), $rowTitleIndex).'"><span class="sandwich" data-key="COLUMN_TITLES" title="'.GetMessage("DATA_EE_ACTIONS_BTN").'"></span></th>';
		foreach($arFields as $k=>$field)
		{
			...
			{
				$sortClass = 'sort_up';
				$sortTitle = GetMessage("DATA_EE_SETTINGS_SORT_ASC");
				if($sortBy==$field)
				{
					if($sortOrder!='DESC')
					{
						$sortClass = 'sort_down';
						$sortTitle = GetMessage("DATA_EE_SETTINGS_SORTED_ASC")."\r\n".GetMessage("DATA_EE_SETTINGS_SORT_DESC");
					}
					else
					{
						$sortTitle = GetMessage("DATA_EE_SETTINGS_SORTED_DESC")."\r\n".$sortTitle;
					}
					$sortClass .= ' active';
				}
			}
			echo '<th>'.
					'<div>'.
						...
						'<a href="javascript:void(0)" class="field_settings '.(empty($EXTRASETTINGS[$list][$k]) ? 'inactive' : '').'" id="field_settings_'.$list.'_'.$k.'" title="'.GetMessage("DATA_EE_SETTINGS_FIELD").'" onclick="EList.ShowFieldSettings(this);">'.
						...
						'</a>'.
						'<a href="javascript:void(0)" class="field_delete" title="'.GetMessage("DATA_EE_SETTINGS_DELETE_FIELD").'" onclick="EList.DeleteColumn(this);"></a>'.
						'<a href="javascript:void(0)" onclick="EList.AddColumn(this);" class="data-ee-new-column" title="'.GetMessage("DATA_EE_SETTINGS_ADD_FIELD").'"></a>'.
					'</div>'.
					'<div>'.
						'<input type="text" name="SETTINGS[FIELDS_LIST_NAMES]['.$list.']['.$k.']" value="'.htmlspecialcharsbx($fieldName).'" class="fieldname" '.CDATAExportUtils::GetCellStyleFormatted($SETTINGS['DISPLAY_PARAMS'][$list]['COLUMN_TITLES'], $SETTINGS_DEFAULT).'>'.
		...
		foreach($arData as $arElement)
		{
			...
			if(isset($arElement['RTYPE']) && ($arElement['RTYPE']=='SECTION_PATH' || preg_match('/^SECTION_(\d+)$/', $arElement['RTYPE'], $m)))
			{
				...
				echo '<td title="'.sprintf(GetMessage("DATA_EE_ROW_NUMBER"), $rowIndex++).'"><span class="sandwich" data-key="'.$arElement['RTYPE'].'" title="'.GetMessage("DATA_EE_ACTIONS_BTN").'"></span></td>';
				echo '<td colspan="'.count($arFields).'"><div class="cell cell_wide"><div class="cell_inner" '.CDATAExportUtils::GetCellStyleFormatted($SETTINGS['DISPLAY_PARAMS'][$list][$arElement['RTYPE']], $SETTINGS_DEFAULT).'>'.$arElement['NAME'].'</div></div></td>';
			}
			else
			{
				echo '<td title="'.sprintf(GetMessage("DATA_EE_ROW_NUMBER"), $rowIndex++).'"></td>';
				foreach($arFields as $key=>$field)
				{
					$val = (isset($arElement[$field.'_'.$key]) ? $arElement[$field.'_'.$key] : $arElement[$field]);
					$isHtml = (bool)(!is_array($val) && strpos($val, 'data-ee-conversion-link')!==false);
					...
					if(((isset($fSettings['INSERT_PICTURE']) && $fSettings['INSERT_PICTURE']=='Y') || $field=='IE_QR_CODE_IMAGE') && $ee->IsPictureField($field))
					{
						...
						foreach($arVals as $mkey=>$val)
						{
							...
							if($ee->IsMultipleField($field))
							{
								...
								{
									if(preg_match('/(<a[^>]+class="data\-ee\-conversion\-link"[^>]*>)(.*)(<\/a>)/Uis', $mval, $m))
									{
							...
							{
								if(preg_match('/(<a[^>]+class="data\-ee\-conversion\-link"[^>]*>)(.*)(<\/a>)/Uis', $val, $m))
								{
									$val = $m[1].'<img src="'.htmlspecialcharsbx($m[2]).'" style="max-width: '.$maxWidth.'px; max-height: '.$maxHeight.'px;">'.$m[3];
								}
								else
					...
					if(is_array($val) && isset($val['TYPE']) && $val['TYPE']=='MULTICELL')
					{
						...
						foreach($val as $kVal=>$vVal)
						{
							...
							$style = (is_array($vVal) && isset($vVal['STYLE']) ? CDATAExportUtils::GetCellStyleFormatted(array_merge($fSettings, $vVal['STYLE']), $SETTINGS_DEFAULT) : '');
						...
						if(strlen($newVal) > 0) $newVal = '<table class="data-ee-multicell">'.$newVal.'</table>';
						$val = $newVal;
						$isHtml = true;
					}
					echo '<td><div class="cell"><div class="cell_inner"'.($SETTINGS_DEFAULT['NOT_SHOW_PREVIEW_STYLES']=='Y' ? '' : ' '.CDATAExportUtils::GetCellStyleFormatted($fSettings, $SETTINGS_DEFAULT)).'>'.($isHtml ? nl2br($val) : nl2br(htmlspecialcharsbx($val))).'</div></div></td>';
				}
			}
			echo '</tr>';
		}
	...
	{
		/*$params = array_merge($SETTINGS_DEFAULT, $SETTINGS);
		$ie = new CDATAExportExcel($DATA_FILE_NAME, $params);
		$ie->Import();
		die();*/
	}
	//*****************************************************************//
...
/////////////////////////////////////////////////////////////////////
$APPLICATION->SetTitle(GetMessage("DATA_EE_PAGE_TITLE").$STEP);
require ($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_admin_after.php");
/*********************************************************************/
/********************  BODY  *****************************************/
/*********************************************************************/
...
$arSubMenu = array();
if($oProfile instanceof CDATAExportProfileDB)
{
	$arSubMenu[] = array(
		"TEXT"=>GetMessage("DATA_EE_MENU_PROFILE_LIST"),
		"TITLE"=>GetMessage("DATA_EE_MENU_PROFILE_LIST"),
		"LINK" => "/bitrix/admin/".$moduleFilePrefix."_profile_list.php?lang=".LANG,
	);
}
$arSubMenu[] = array(
	"TEXT"=>GetMessage("DATA_EE_SHOW_CRONTAB"),
	"TITLE"=>GetMessage("DATA_EE_SHOW_CRONTAB"),
	"ONCLICK" => "EProfile.ShowCron();",
);
$aMenu = array(
	array(
		"TEXT"=>GetMessage("DATA_EE_MENU_HELP"),
		"TITLE"=>GetMessage("DATA_EE_MENU_HELP"),
		"ONCLICK" => "EHelper.ShowHelp();",
		"ICON" => "",
	),
	array(
		"TEXT"=>GetMessage("DATA_EE_TOOLS_LIST"),
		"TITLE"=>GetMessage("DATA_EE_TOOLS_LIST"),
		"MENU" => $arSubMenu,
		"ICON" => "btn_green",
	)
);
...
if ($STEP < 2)
{
	...
	if(!empty($arProfiles))
	{
		...
		{
			$message .= '<div class="data-proccess-item">'.GetMessage("DATA_EE_PROCESSED_PROFILE").': '.$v['name'].' ('.GetMessage("DATA_EE_PROCESSED_PERCENT_LOADED").' '.$v['percent'].'%). &nbsp; &nbsp; &nbsp; &nbsp; <a href="javascript:void(0)" onclick="EProfile.ContinueProccess(this, '.$v['key'].')">'.GetMessage("DATA_EE_PROCESSED_CONTINUE").'</a> &nbsp; <a href="javascript:void(0)" onclick="EProfile.RemoveProccess(this, '.$v['key'].')">'.GetMessage("DATA_EE_PROCESSED_DELETE").'</a></div>';
		}
		echo '<div style="display: none;">';
		CAdminMessage::ShowMessage(array(
			'TYPE' => 'error',
			'MESSAGE' => GetMessage("DATA_EE_PROCESSED_TITLE"),
			'DETAILS' => $message,
			'HTML' => true
		));
		echo '</div>';
...
<form method="POST" action="<?echo $sDocPath ?>?lang=<?echo LANG ?>" ENCTYPE="multipart/form-data" name="dataload" id="dataload" class="data-ee-s1-form">
...
$aTabs = array(
	array(
		"DIV" => "edit1",
		"TAB" => GetMessage("DATA_EE_TAB1") ,
		"ICON" => "iblock",
		"TITLE" => GetMessage("DATA_EE_TAB1_ALT"),
	) ,
	array(
		"DIV" => "edit2",
		"TAB" => GetMessage("DATA_EE_TAB2") ,
		"ICON" => "iblock",
		"TITLE" => sprintf(GetMessage("DATA_EE_TAB2_ALT"), (isset($arProfile['NAME']) ? $arProfile['NAME'] : '')),
	) ,
	array(
		"DIV" => "edit3",
		"TAB" => GetMessage("DATA_EE_TAB3") ,
		"ICON" => "iblock",
		"TITLE" => sprintf(GetMessage("DATA_EE_TAB3_ALT"), (isset($arProfile['NAME']) ? $arProfile['NAME'] : '')),
	) ,
);
...
?>
	...
	<tr class="heading">
		<td colspan="2" class="data-ee-profile-header">
			<div>
				<?echo GetMessage("DATA_EE_PROFILE_HEADER"); ?>
				<a href="javascript:void(0)" onclick="EHelper.ShowHelp();" title="<?echo GetMessage("DATA_EE_MENU_HELP"); ?>" class="data-ee-help-link"></a>
			</div>
		</td>
	</tr>

	<tr>
		<td><?echo GetMessage("DATA_EE_PROFILE"); ?>:</td>
		<td>
			<?$oProfile->ShowProfileList('PROFILE_ID');?>
			
			<?if(strlen($PROFILE_ID) > 0 && $PROFILE_ID!='new'){?>
				<span class="data-ee-edit-btns">
					<a href="javascript:void(0)" class="adm-table-btn-edit" onclick="EProfile.ShowRename();" title="<?echo GetMessage("DATA_EE_RENAME_PROFILE");?>" id="action_edit_button"></a>
					<a href="javascript:void(0);" class="adm-table-btn-copy" onclick="EProfile.Copy();" title="<?echo GetMessage("DATA_EE_COPY_PROFILE");?>" id="action_copy_button"></a>
					<a href="javascript:void(0);" class="adm-table-btn-delete" onclick="if(confirm('<?echo GetMessage("DATA_EE_DELETE_PROFILE_CONFIRM");?>')){EProfile.Delete();}" title="<?echo GetMessage("DATA_EE_DELETE_PROFILE");?>" id="action_delete_button"></a>
				</span>
			<?}?>
		</td>
	</tr>
	...
	<tr id="new_profile_name">
		<td><?echo GetMessage("DATA_EE_NEW_PROFILE_NAME"); ?>:</td>
		<td>
	...
	?>
		<tr class="heading">
			<td colspan="2"><?echo GetMessage("DATA_EE_DEFAULT_SETTINGS"); ?></td>
		</tr>
		...
		<tr>
			...
			$xlsxShow = true;
			?>
			<td width="40%"><?echo GetMessage("DATA_EE_FILE_EXT"); ?></td>
			<td width="60%" class="data-ie-file-choose">
				<select name="SETTINGS_DEFAULT[FILE_EXTENSION]" id="data-ee-file-extension">
					<?if($xlsxShow){?>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_FILE_PATH"); ?>:  </td>
			<td>
				<?
				...
				?>
				<input type="text" name="SETTINGS_DEFAULT[FILE_PATH]" id="data-ee-file-path" value="<?echo htmlspecialcharsbx($path); ?>" size="55">
			</td>
		</tr>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_ELEMENT_MULTIPLE_SEPARATOR"); ?>:</td>
			<td>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_MAX_SECTION_LEVEL"); ?>:  <span id="hint_MAX_SECTION_LEVEL"></span><script>BX.hint_replace(BX('hint_MAX_SECTION_LEVEL'), '<?echo GetMessage("DATA_EE_MAX_SECTION_LEVEL_HINT"); ?>');</script></td>
			<td>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_EXPORT_FIELD_CODES"); ?>:</td>
			<td>
		...
		<tr class="heading" id="csv_settings_block">
			<td colspan="2"><?echo GetMessage("DATA_EE_SETTINGS_CSV"); ?></td>
		</tr>
		...
		?>
		<tr>
			<td><?echo GetMessage("DATA_EE_CSV_SEPARATOR"); ?>:</td>
			<td>
				<select name="SETTINGS_DEFAULT[CSV_SEPARATOR]">
					<option value=";" <?if($SETTINGS_DEFAULT['CSV_SEPARATOR']==';'){echo 'selected';}?>><?echo GetMessage("DATA_EE_CSV_SEPARATOR_SEMICOLON"); ?></option>
					<option value="," <?if($SETTINGS_DEFAULT['CSV_SEPARATOR']==','){echo 'selected';}?>><?echo GetMessage("DATA_EE_CSV_SEPARATOR_COMMA"); ?></option>
					<option value="\t" <?if($SETTINGS_DEFAULT['CSV_SEPARATOR']=='\t'){echo 'selected';}?>><?echo GetMessage("DATA_EE_CSV_SEPARATOR_TAB"); ?></option>
				</select>
			</td>
		</tr>
		
		<tr>
			<td><?echo GetMessage("DATA_EE_CSV_ENCLOSURE"); ?>:</td>
			<td>
				<select name="SETTINGS_DEFAULT[CSV_ENCLOSURE]" <?if($csvYandex){echo 'disabled';}?>>
					<option value='"' <?if($SETTINGS_DEFAULT['CSV_ENCLOSURE']=='"'){echo 'selected';}?>><?echo GetMessage("DATA_EE_CSV_ENCLOSURE_DOUBLE_QUOTE"); ?></option>
					<option value="'" <?if($SETTINGS_DEFAULT['CSV_ENCLOSURE']=="'"){echo 'selected';}?>><?echo GetMessage("DATA_EE_CSV_ENCLOSURE_SINGLE_QUOTE"); ?></option>
					<option value="" <?if($SETTINGS_DEFAULT['CSV_ENCLOSURE']===""){echo 'selected';}?>><?echo GetMessage("DATA_EE_CSV_ENCLOSURE_EMPTY"); ?></option>
				</select>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_CSV_ENCODING"); ?>:</td>
			<td>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_CSV_YANDEX"); ?>:</td>
			<td>
		...
		<tr class="heading">
			<td colspan="2"><?echo GetMessage("DATA_EE_SETTINGS_SECTIONS"); ?></td>
		</tr>
		
		<tr>
			<td><?echo GetMessage("DATA_EE_EXPORT_ELEMENT_ONE_SECTION"); ?>:</td>
			<td>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_EXPORT_SECTIONS_ONE_CELL"); ?>:</td>
			<td>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_EXPORT_SEP_SECTIONS"); ?>:</td>
			<td>
		...
		<tr <?if($SETTINGS_DEFAULT['EXPORT_SEP_SECTIONS']!='Y'){echo 'style="display: none;"';}?>>
			<td><?echo GetMessage("DATA_EE_EXPORT_SECTION_PATH"); ?>:</td>
			<td>
		...
		<tr <?if($SETTINGS_DEFAULT['EXPORT_SEP_SECTIONS']!='Y'){echo 'style="display: none;"';}?>>
			<td><?echo GetMessage("DATA_EE_EXPORT_GROUP_PRODUCTS"); ?>:</td>
			<td>
		...
		<tr <?if($SETTINGS_DEFAULT['EXPORT_SEP_SECTIONS']!='Y'){echo 'style="display: none;"';}?>>
			<td><?echo GetMessage("DATA_EE_EXPORT_GROUP_SUBSECTIONS"); ?>:</td>
			<td>
		...
		<tr <?if($SETTINGS_DEFAULT['EXPORT_SEP_SECTIONS']!='Y'){echo 'style="display: none;"';}?>>
			<td><?echo GetMessage("DATA_EE_EXPORT_GROUP_OPEN"); ?>:</td>
			<td>
		...
		<tr <?if($SETTINGS_DEFAULT['EXPORT_SEP_SECTIONS']!='Y'){echo 'style="display: none;"';}?>>
			<td><?echo GetMessage("DATA_EE_EXPORT_GROUP_INDENT"); ?>:</td>
			<td>
		...
		<tr <?if($SETTINGS_DEFAULT['EXPORT_SEP_SECTIONS']!='Y'){echo 'style="display: none;"';}?>>
			<td><?echo GetMessage("DATA_EE_EXPORT_SEP_SECTIONS_SORT"); ?>:</td>
			<td>
				<select name="SETTINGS_DEFAULT[EXPORT_SEP_SECTIONS_SORT]">
					<option value=""><?echo GetMessage("DATA_EE_EXPORT_SEP_SECTIONS_SORT_DEFAULT");?></option>
					<option value="NAME"<?if($SETTINGS_DEFAULT['EXPORT_SEP_SECTIONS_SORT']=='NAME'){echo 'selected';}?>><?echo GetMessage("DATA_EE_EXPORT_SEP_SECTIONS_SORT_NAME");?></option>
				</select>
			</td>
		</tr>
		
		<?
		$OFFERS_IBLOCK_ID = CDATAExportUtils::GetOfferIblock($SETTINGS_DEFAULT['IBLOCK_ID']);
		?>
		<tr class="heading data-sku-block" <?if(!$OFFERS_IBLOCK_ID){echo 'style="display: none;"';}?> >
			<td colspan="2"><?echo GetMessage("DATA_EE_SETTINGS_OFFERS"); ?> <a href="javascript:void(0)" onclick="EProfile.ToggleAdditionalSettings(this)" class="data-head-more show"><?echo GetMessage("DATA_EE_SETTINGS_ADDITONAL_SHOW_HIDE"); ?></a></td>
		</tr>
		
		<tr class="data-sku-block" <?if(!$OFFERS_IBLOCK_ID){echo 'style="display: none;"';}?> >
			<td><?echo GetMessage("DATA_EE_EXPORT_ONE_OFFER_MIN_PRICE"); ?>:</td>
			<td>
				<table cellspacing="0"><tr>
					<td style="padding-left: 0px;">
						<input type="checkbox" name="SETTINGS_DEFAULT[EXPORT_ONE_OFFER_MIN_PRICE]" value="Y" <?if($SETTINGS_DEFAULT['EXPORT_ONE_OFFER_MIN_PRICE']=='Y'){echo 'checked';}?> onclick="if(this.checked){$('#data_ee_offerminpricetype').show();}else{$('#data_ee_offerminpricetype').hide();}">
					</td>
					<td>&nbsp; &nbsp;</td>
					<td id="data_ee_offerminpricetype" <?if($SETTINGS_DEFAULT['EXPORT_ONE_OFFER_MIN_PRICE']!='Y'){echo 'style="display: none;"';}?>>
					<?
					if($bCatalog)
					{
						$fl = new CDATAEEFieldList($SETTINGS_DEFAULT);
						...
						?>
						<?echo GetMessage("DATA_EE_EXPORT_ONE_OFFER_MIN_PRICE_TYPE"); ?><br>
		...
		<tr class="data-sku-block" <?if(!$OFFERS_IBLOCK_ID){echo 'style="display: none;"';}?> >
			<td><?echo GetMessage("DATA_EE_EXPORT_OFFERS_JOIN"); ?>:</td>
			<td>
		...
		<tr class="data-sku-block" <?if(!$OFFERS_IBLOCK_ID){echo 'style="display: none;"';}?> >
			<td><?echo GetMessage("DATA_EE_EXPORT_PRODUCTS_JOIN"); ?>:</td>
			<td>
		...
		<tr class="heading">
			<td colspan="2"><?echo GetMessage("DATA_EE_SETTINGS_DISPLAY"); ?> <a href="javascript:void(0)" onclick="EProfile.ToggleAdditionalSettings(this)" class="data-head-more show"><?echo GetMessage("DATA_EE_SETTINGS_ADDITONAL_SHOW_HIDE"); ?></a></td>
		</tr>
		
		<tr>
			<td><?echo GetMessage("DATA_EE_DISPLAY_LOCK_HEADERS"); ?>:</td>
			<td>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_EXPORT_ROW_AUTO_HEIGHT"); ?>:</td>
			<td>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_EXPORT_ROW_MIN_HEIGHT"); ?>:</td>
			<td>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_DISPLAY_TEXT_ALIGN"); ?>:</td>
			<td>
				<select name="SETTINGS_DEFAULT[DISPLAY_TEXT_ALIGN]">
					<option value="LEFT" <?if($SETTINGS_DEFAULT['DISPLAY_TEXT_ALIGN']=='LEFT'){echo 'selected';}?>><?echo GetMessage("DATA_EE_DISPLAY_TEXT_ALIGN_LEFT"); ?></option>
					<option value="CENTER" <?if($SETTINGS_DEFAULT['DISPLAY_TEXT_ALIGN']=='CENTER'){echo 'selected';}?>><?echo GetMessage("DATA_EE_DISPLAY_TEXT_ALIGN_CENTER"); ?></option>
					<option value="RIGHT" <?if($SETTINGS_DEFAULT['DISPLAY_TEXT_ALIGN']=='RIGHT'){echo 'selected';}?>><?echo GetMessage("DATA_EE_DISPLAY_TEXT_ALIGN_RIGHT"); ?></option>
				</select>
			</td>
		</tr>
		
		<tr>
			<td><?echo GetMessage("DATA_EE_DISPLAY_VERTICAL_ALIGN"); ?>:</td>
			<td>
				<select name="SETTINGS_DEFAULT[DISPLAY_VERTICAL_ALIGN]">
					<option value="TOP" <?if($SETTINGS_DEFAULT['DISPLAY_VERTICAL_ALIGN']=='TOP'){echo 'selected';}?>><?echo GetMessage("DATA_EE_DISPLAY_VERTICAL_ALIGN_TOP"); ?></option>
					<option value="CENTER" <?if($SETTINGS_DEFAULT['DISPLAY_VERTICAL_ALIGN']=='CENTER'){echo 'selected';}?>><?echo GetMessage("DATA_EE_DISPLAY_VERTICAL_ALIGN_CENTER"); ?></option>
					<option value="BOTTOM" <?if($SETTINGS_DEFAULT['DISPLAY_VERTICAL_ALIGN']=='BOTTOM'){echo 'selected';}?>><?echo GetMessage("DATA_EE_DISPLAY_VERTICAL_ALIGN_BOTTOM"); ?></option>
				</select>
			</td>
		</tr>
		
		<tr>
			<td><?echo GetMessage("DATA_EE_DISPLAY_SETTING_FONT_FAMILY");?>:</td>
			<td>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_DISPLAY_SETTING_FONT_SIZE");?>:</td>
			<td>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_DISPLAY_SETTING_FONT_COLOR");?>:</td>
			<td>
		...
		<?/*?><tr>
			<td><?echo GetMessage("DATA_EE_DISPLAY_SETTING_BACKGROUND_COLOR");?>:</td>
			<td>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_DISPLAY_SETTING_FONT_STYLE_BOLD");?>:</td>
			<td>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_DISPLAY_SETTING_FONT_STYLE_ITALIC");?>:</td>
			<td>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_DISPLAY_SETTING_BORDER_STYLE");?>:</td>
			<td>
				<select name="SETTINGS_DEFAULT[BORDER_STYLE]">
					<option value="NONE" <?if($SETTINGS_DEFAULT['BORDER_STYLE']=='NONE'){echo 'selected';}?>><?echo GetMessage("DATA_EE_DISPLAY_SETTING_BORDER_STYLE_NONE"); ?></option>
					<option value="THIN" <?if($SETTINGS_DEFAULT['BORDER_STYLE']=='THIN'){echo 'selected';}?>><?echo GetMessage("DATA_EE_DISPLAY_SETTING_BORDER_STYLE_THIN"); ?></option>
					<option value="MEDIUM" <?if($SETTINGS_DEFAULT['BORDER_STYLE']=='MEDIUM'){echo 'selected';}?>><?echo GetMessage("DATA_EE_DISPLAY_SETTING_BORDER_STYLE_MEDIUM"); ?></option>
					<option value="THICK" <?if($SETTINGS_DEFAULT['BORDER_STYLE']=='THICK'){echo 'selected';}?>><?echo GetMessage("DATA_EE_DISPLAY_SETTING_BORDER_STYLE_THICK"); ?></option>
				</select>
			</td>
		</tr>
		
		<tr>
			<td><?echo GetMessage("DATA_EE_DISPLAY_SETTING_BORDER_COLOR");?>:</td>
			<td>
		...
		<tr class="heading">
			<td colspan="2"><?echo GetMessage("DATA_EE_DOCUMENT_PARAMS"); ?> <a href="javascript:void(0)" onclick="EProfile.ToggleAdditionalSettings(this)" class="data-head-more show"><?echo GetMessage("DATA_EE_SETTINGS_ADDITONAL_SHOW_HIDE"); ?></a></td>
		</tr>
		
		<tr>
			<td><?echo GetMessage("DATA_EE_DOCPARAM_TITLE");?>:</td>
			<td>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_DOCPARAM_SUBJECT");?>:</td>
			<td>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_DOCPARAM_AUTHOR");?>:</td>
			<td>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_DOCPARAM_ORG");?>:</td>
			<td>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_DOCPARAM_CATEGORY");?>:</td>
			<td>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_DOCPARAM_KEYWORDS");?>:</td>
			<td>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_DOCPARAM_DESCRIPTION");?>:</td>
			<td>
		...
		<tr class="heading">
			<td colspan="2"><?echo GetMessage("DATA_EE_EXT_SERVICES"); ?> <a href="javascript:void(0)" onclick="EProfile.ToggleAdditionalSettings(this)" class="data-head-more show"><?echo GetMessage("DATA_EE_SETTINGS_ADDITONAL_SHOW_HIDE"); ?></a></td>
		</tr>
		
		<tr>
			<td><?echo GetMessage("DATA_EE_EXT_SERVICES_BX24");?>:</td>
			<td>
				<table cellspacing="0"><tr>
				...
				<td id="bx24p"<?if($SETTINGS_DEFAULT['EXPORT_TO_BX24']!='Y'){echo ' style="display: none;"';}?>>
					<div id="bx24_help" style="display: none;">
						...
						<?echo sprintf(GetMessage("DATA_EE_EXT_SERVICES_BX24_REST_URL_HELP"), sprintf($imgStr, 'bx24_1.jpg'), sprintf($imgStr, 'bx24_2.jpg'), sprintf($imgStr, 'bx24_3.jpg'));?>
					</div>
					<div>
						<?echo GetMessage("DATA_EE_EXT_SERVICES_BX24_REST_URL");?>:
						...
						<?echo sprintf(GetMessage("DATA_EE_EXT_SERVICES_BX24_REST_URL_EXAMPLE"), 'https://b24-b41cc2.bitrix24.ru/rest/1/e9pzrkwg0aijfh9m/');?> &nbsp; (<a href="javascript:void(0)" id="bx24_help_link"><?echo GetMessage("DATA_EE_EXT_SERVICES_BX24_REST_URL_HOW_GET");?></a>)
					</div>
					<br>
					<div>
						<?echo GetMessage("DATA_EE_EXT_SERVICES_BX24_FOLDER_ID");?>:
						<div>
						...
						<div id="bx24_folder_struct" class="data-ee-bx24-struct"><select></select></div>
					</div>
					<br>
					<div>
						<?echo GetMessage("DATA_EE_EXT_SERVICES_BX24_MODE");?>:
						<div>
							<input type="radio" name="SETTINGS_DEFAULT[BX24_MODE]" value="UPDATE"<?if($SETTINGS_DEFAULT['BX24_MODE']!='REPLACE'){echo ' checked';}?> id="bx24_mode_update"> <label for="bx24_mode_update"><?echo GetMessage("DATA_EE_EXT_SERVICES_BX24_MODE_UPDATE");?></label>
							<br>
							<input type="radio" name="SETTINGS_DEFAULT[BX24_MODE]" value="REPLACE"<?if($SETTINGS_DEFAULT['BX24_MODE']=='REPLACE'){echo ' checked';}?> id="bx24_mode_replace"> <label for="bx24_mode_replace"><?echo GetMessage("DATA_EE_EXT_SERVICES_BX24_MODE_REPLACE");?></label>
						</div>
					</div>
				</td>
				</tr></table>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_EXT_SERVICES_YADISK");?>:</td>
			<td>
				<table cellspacing="0"><tr>
				...
				<td id="yadiskp"<?if($SETTINGS_DEFAULT['EXPORT_TO_YADISK']!='Y'){echo ' style="display: none;"';}?>>
					<div>
						<?echo GetMessage("DATA_EE_EXT_SERVICES_YADISK_TOKEN");?>:
						<div><input type="text" name="SETTINGS_DEFAULT[YADISK_TOKEN]" value="<?=htmlspecialcharsbx($SETTINGS_DEFAULT['YADISK_TOKEN'])?>" size="50"> &nbsp; <a href="https://oauth.yandex.ru/authorize?response_type=token&client_id=9169c49a80754cf3a89e11c274870299" target="_blank"><?echo GetMessage("DATA_EE_EXT_SERVICES_YADISK_TOKEN_GET");?></a></div>
					</div>
					<br>
					<div>
						<?echo GetMessage("DATA_EE_EXT_SERVICES_YADISK_PATH");?>:
						<div><input type="text" name="SETTINGS_DEFAULT[YADISK_PATH]" value="<?=htmlspecialcharsbx($SETTINGS_DEFAULT['YADISK_PATH'])?>" size="50"></div>
						<?echo GetMessage("DATA_EE_EXT_SERVICES_YADISK_PATH_EXAMPLE");?>
					</div>
				</td>
				</tr></table>
			</td>
		...
		<tr class="heading">
			<td colspan="2"><?echo GetMessage("DATA_EE_OTHER_SETTINGS"); ?> <a href="javascript:void(0)" onclick="EProfile.ToggleAdditionalSettings(this)" class="data-head-more show"><?echo GetMessage("DATA_EE_SETTINGS_ADDITONAL_SHOW_HIDE"); ?></a></td>
		</tr>
		
		<tr>
			<td><?echo GetMessage("DATA_EE_OTHER_EXPORT_FILES_IN_ARCHIVE");?>:</td>
			<td>
				<table cellspacing="0"><tr>
				<td style="padding-left: 0px;">
				...
				<td>
					<div id="EXPORT_FILES_IN_ARCHIVE_wrap" <?if($SETTINGS_DEFAULT['EXPORT_FILES_IN_ARCHIVE']!='Y'){echo 'style="display: none;"';}?>>
						<?echo GetMessage("DATA_EE_OTHER_FILES_ARCHIVE_PATH");?><br>
						<?
						...
						?>
						<input type="text" name="SETTINGS_DEFAULT[FILES_ARCHIVE_PATH]" id="data-ee-file-path" value="<?echo htmlspecialcharsbx($archivePath); ?>" size="55">
					</div>
				</td>
				</tr></table>
			</td>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_OTHER_SETTINGS_MERGE_SHEETS");?>:</td>
			<td>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_OTHER_SETTINGS_NOT_SHOW_PREVIEW_STYLES");?>: <span id="hint_NOT_SHOW_PREVIEW_STYLES"></span><script>BX.hint_replace(BX('hint_NOT_SHOW_PREVIEW_STYLES'), '<?echo GetMessage("DATA_EE_OTHER_SETTINGS_NOT_SHOW_PREVIEW_STYLES_HINT"); ?>');</script></td>
			<td>
		...
		<tr>
			<td><?echo GetMessage("DATA_EE_OTHER_SETTINGS_COUNT_SHOW_LINES");?>:</td>
			<td>
...
?>
	...
	<tr>
		<td colspan="2" id="data-ee-sheet-list">
			<?
			$arKeys = array(0);
			...
			{
				?>
				<div class="data-ee-sheet-wrap<?if($ind > 0){echo ' withmargin';}?>">
					<?
						/*echo '<div id="tree"></div>';
						$obCond = new CDATAExportCondTree();
					...
					?>
					<div class="data-ee-sheet" 
						id="data-ee-sheet-<?echo $list;?>" 
						data-sheet-index="<?echo $list;?>" 
						data-show-field-codes="<?echo $showCodes;?>">
					</div>
					<div class="data-ee-new-list-wrap" style="display: none;">
						<input type="button" value="<?echo GetMessage("DATA_EE_ADD_NEW_LIST"); ?>" title="<?echo GetMessage("DATA_EE_ADD_NEW_LIST_HINT");?>">
					</div>
				</div>
				<?
				$ind++;
			...
			?>
			<?/*?><div class="data-ee-new-list-wrap">
				<input type="button" value="<?echo GetMessage("DATA_EE_ADD_NEW_LIST"); ?>" onclick="EList.AddNewList(this);">
			</div><?*/?>
		</td>
	</tr>
...
?>
	<tr>
		<td id="resblock" class="data-ee-result">
		 <table width="100%"><tr><td width="50%">
			<div id="progressbar"><span class="pline"></span><span class="presult load"><b>0%</b><span 
				data-prefix="<?echo GetMessage("DATA_EE_READ_LINES"); ?>" 
				data-export="<?echo GetMessage("DATA_EE_STATUS_EXPORT"); ?>" 
			><?echo GetMessage("DATA_EE_EXPORT_INIT"); ?></span></span></div>

			<div id="block_error_import" style="display: none;">
				<?echo CAdminMessage::ShowMessage(array(
					"TYPE" => "ERROR",
					"MESSAGE" => GetMessage("DATA_EE_IMPORT_ERROR_CONNECT"),
					/*"DETAILS" => '<div><a href="javascript:void(0)" onclick="EProfile.ContinueProccess(this, '.$PROFILE_ID.');">'.GetMessage("DATA_EE_PROCESSED_CONTINUE").'</a><br><br>'.sprintf(GetMessage("DATA_EE_IMPORT_ERROR_CONNECT_COMMENT"), '/bitrix/admin/settings.php?lang=ru&mid='.$moduleId.'&mid_menu=1').'</div>',*/
					"DETAILS" => '<div>'.(COption::GetOptionString($moduleId, 'AUTO_CONTINUE_EXPORT', 'N')=='Y' ? sprintf(GetMessage("DATA_EE_EXPORT_AUTO_CONTINUE"), '<span id="data_ee_auto_continue_time"></span>').'<br>' : '').'<a href="javascript:void(0)" onclick="EProfile.ContinueProccess(this, '.$PROFILE_ID.');">'.GetMessage("DATA_EE_PROCESSED_CONTINUE").'</a></div>',
					"HTML" => true,
				))?>
			</div>
			...
			<div id="block_error" style="display: none;">
				<?echo CAdminMessage::ShowMessage(array(
					"TYPE" => "ERROR",
					"MESSAGE" => GetMessage("DATA_EE_IMPORT_ERROR"),
			...
			<div class="detail_status">
				<?
				$outputFile = CDATAExportUtils::PrepareExportFileName($SETTINGS_DEFAULT['FILE_PATH']);
				...
				echo CAdminMessage::ShowMessage(array(
					"TYPE" => "PROGRESS",
					"MESSAGE" => '<!--<div id="res_continue">'.GetMessage("DATA_EE_AUTO_REFRESH_CONTINUE").'</div><div id="res_finish" style="display: none;">'.GetMessage("DATA_EE_SUCCESS").'</div>-->',
					"DETAILS" =>

					GetMessage("DATA_EE_SU_ALL").' <b id="total_read_line">0</b><br>'.
					/*.GetMessage("DATA_EE_SU_ELEMENT_ADDED").' <b id="element_added_line">0</b><br>'.
					(!empty($SETTINGS_DEFAULT['ELEMENT_UID_SKU']) ? (GetMessage("DATA_EE_SU_SKU_ADDED").' <b id="sku_added_line">0</b><br>') : '').
					GetMessage("DATA_EE_SU_SECTION_ADDED").' <b id="section_added_line">0</b><br>'.*/
					' <span id="data_ee_ready_file" style="visibility: hidden;"><br>'.GetMessage("DATA_EE_READY_FILE_LINK").' <br><a href="'.htmlspecialcharsbx($outputFile).'?hash='.md5(mt_rand()).'" target="_blank">'.$outputFile.'</a> <br><br><a href="/bitrix/admin/fileman_file_download.php?path='.htmlspecialcharsbx($outputFile).'&lang='.LANGUAGE_ID.'&hash='.md5(mt_rand()).'">'.GetMessage("DATA_EE_DOWNLOAD_FILE").'</a></span><br>',
					"HTML" => true,
				));
				?>
			</div>
...
<?
if($STEP == 2){ ?>
<input type="submit" name="backButton" value="&lt;&lt; <?echo GetMessage("DATA_EE_BACK"); ?>">
<input type="submit" name="saveConfigButton" value="<?echo GetMessage("DATA_EE_SAVE_CONFIGURATION"); ?>" style="float: right;">
<?
}
...
?>
	...
	<input type="submit" value="<?echo ($STEP == 2) ? GetMessage("DATA_EE_NEXT_STEP_F") : GetMessage("DATA_EE_NEXT_STEP"); ?> &gt;&gt;" name="submit_btn" class="adm-btn-save">
<? 
}
else
{
?>
	...
	<input type="submit" name="backButton2" value="&lt;&lt; <?echo GetMessage("DATA_EE_2_1_STEP"); ?>" class="adm-btn-save">
<?
}
?>
