...
class CDATAExportExcelWriterXlsx {
					...
					foreach($arFields as $k=>$field)
					{
						...
						foreach($arVals2 as $mkey=>$val)
						{
							...
							foreach($arCurImgVals as $key=>$val)
							{
								if(!$val) continue;
								$link = '';
								if(preg_match('/<a[^>]+class="data\-ee\-conversion\-link"[^>]+href="([^"]*)"[^>]*>(.*)<\/a>/Uis', $val, $m))
								{
									$link = $m[1];
									$val = $m[2];
								}
    ...
    public function writeStringCell($value, $colspan=1, $arStyles=array(), $isData=false, $cellType='')
    {
        ...
        if (1) {
			...
			if(isset($this->linkCells[$cell - 1]) && preg_match('/<a[^>]+class="data\-ee\-conversion\-link"[^>]+href="([^"]*)"[^>]*>(.*)<\/a>/Uis', $value, $m))
			{
		...
		if(!empty($textRows))
		{
			foreach($textRows as $k=>$v)
			{
				...
				if(preg_match('/^\[\[(\d+)\]\]$/', $v, $m))
				{
					...
					$arFileOrig = CDATAExportUtils::GetFileArray($fileId);
    ...
    public function closeExcelWriter($break = false)
    {
		...
		if(\CDATAExportUtils::CanUseZipArchive() && ($zipObj = new ZipArchive()) && $zipObj->open($this->outputFile, ZipArchive::OVERWRITE|ZipArchive::CREATE)===true)
		{
