...
class CDATAExportProfileDB extends CDATAExportProfileAll {
	...
	function __construct($suffix='')
	{
		...
		foreach(array($this->tmpdir, $this->uploadDir) as $k=>$v)
		{
			...
			{
				$this->errors[] = sprintf(Loc::getMessage('DATA_EE_DIR_NOT_WRITABLE'), $v);
			}
		}
	...
	public function CheckStorage()
	{
		...
		if(!$conn->isTableExists($tblName))
		{
			...
			{
				$profileFs = new CDATAExportProfileFS($this->suffix);
	...
	private function GetEntity()
	{
		if(!$this->entity)
		{
			...
			{
				$this->entity = new \Bitrix\DataExportexcel\ProfileHlTable();
			}
			else
			{
				$this->entity = new \Bitrix\DataExportexcel\ProfileTable();
			}
		}
		return $this->entity;
	}
	...
	public function Add($name)
	{
		...
		if(strlen($name)==0)
		{
			$APPLICATION->throwException(Loc::getMessage("DATA_EE_NOT_SET_PROFILE_NAME"));
			return false;
		}
		...
		{
			$APPLICATION->throwException(Loc::getMessage("DATA_EE_PROFILE_NAME_EXISTS"));
			return false;
		}
	...
	public function Copy($ID)
	{
		...
		$newName = $arProfile['NAME'].Loc::getMessage("DATA_EE_PROFILE_COPY");
	...
	public function GetStatus($id, $bImported=false)
	{
		...
		if(!file_exists($tmpfile))
		{
			if($bImported)
			{
				if(empty($arProfile) || !empty($arProfile['DATE_FINISH'])) return array('STATUS'=>'OK', 'MESSAGE'=>Loc::getMessage("DATA_EE_STATUS_COMPLETE"));
				//else return array('STATUS'=>'ERROR', 'MESSAGE'=>Loc::getMessage("DATA_EE_STATUS_FILE_ERROR"));
		...
		$status = 'OK';
		if((time() - filemtime($tmpfile) < 4*60)) $statusText = Loc::getMessage("DATA_EE_STATUS_PROCCESS");
		else 
		{
			$statusText = Loc::getMessage("DATA_EE_STATUS_BREAK");
			$status = 'ERROR';
		}
	...
	public function OutputBackup()
	{
		...
		file_put_contents($arFiles['config'], base64_encode(serialize(
			array(
				...
				'encoding' => \CDATAExportUtils::getSiteEncoding()
			)
		)));
	...
	public function RestoreBackup($arPFile, $arParams)
	{
		...
		{
			return array('TYPE'=>'ERROR', 'MESSAGE'=>Loc::getMessage("DATA_IE_RESTORE_NOT_LOAD_FILE"));
		}
		$filename = $arPFile['name'];
		if(ToLower(\CDATAExportUtils::GetFileExtension($filename))!=='zip')
		{
			return array('TYPE'=>'ERROR', 'MESSAGE'=>Loc::getMessage("DATA_IE_RESTORE_FILE_NOT_VALID"));
		}
		...
		return array('TYPE'=>'SUCCESS', 'MESSAGE'=>Loc::getMessage("DATA_IE_RESTORE_SUCCESS"));
	}
}