...
class CDATAExportExcel {
	...
	function __construct($params=array(), $fparams=array(), $stepparams=false, $pid = false)
	{
		...
		if(is_array($stepparams))
		{
			...
			$oProfile = CDATAExportProfile::getInstance();
	...
	public function Export()
	{
		...
		CDATAExportUtils::PrepareTextRows($this->params['TEXT_ROWS_TOP'], $this->params, $this->stepparams);
		CDATAExportUtils::PrepareTextRows($this->params['TEXT_ROWS_TOP2'], $this->params, $this->stepparams);

		$filePath = CDATAExportUtils::PrepareExportFileName($this->params['FILE_PATH']);
		...
		if($this->params['FILE_EXTENSION']=='xlsx')
		{
			...
			{
				$objWriter = new CDATAExportExcelWriterXlsx($arWriterParams, $this);
			}
		...
		elseif($this->params['FILE_EXTENSION']=='csv')
		{
			...
			if(!is_object($objWriter))
			{
				$objWriter = new CDATAExportExcelWriterCsv($arWriterParams, $this);
			}
			if(false===$objWriter->Save()/* && $this->CheckTimeEnding()*/)
			{
				$writerFileParams = $this->tmpdir.'writer_params.txt';
		...
		elseif($this->params['FILE_EXTENSION']=='dbf')
		{
			...
			if(!is_object($objWriter))
			{
				$objWriter = new CDATAExportExcelWriterDbf($arWriterParams, $this);
			}
			if(false===$objWriter->Save()/* && $this->CheckTimeEnding()*/)
			{
				$writerFileParams = $this->tmpdir.'writer_params.txt';
		...
		else
		{
			...
			$objWriter = DATAPHPExcel_IOFactory::createWriter($objPHPExcel, $writerType);
			if($writerType == 'CSV')
			{
	...
	public function GetFileArray($file, $arDef=array())
	{
		...
		elseif(preg_match('/http(s)?:\/\//', $file))
		{
			...
			if(preg_match('/[^A-Za-z0-9\-\.]/', $arUrl['host']))
			{
				...
				if(class_exists('idna_convert'))
				{
					...
					$oldHost = $arUrl['host'];
					if(!CUtil::DetectUTF8($oldHost)) $oldHost = CDATAExportUtils::Win1251Utf8($oldHost);
		...
		{
			$file = CDATAExportUtils::Win1251Utf8($file);
			$arFile = CFile::MakeFileArray($file);
		}
	...
	{
		$trueVals = array_map('trim', explode(',', GetMessage("DATA_EE_FIELD_VAL_Y")));
		$falseVals = array_map('trim', explode(',', GetMessage("DATA_EE_FIELD_VAL_N")));
	...
	public function GetFileValue($val, $key=false)
	{
		if($val)
		{
			if(is_numeric($val))
			{
				$arFile = CDATAExportUtils::GetFileArray($val);
				if($arFile)
				{
					$val = $arFile['SRC'];
				}
	...
	public function PutFileToArchive($val, $removePath='')
	{
		...
		$fsEncoding = CDATAExportUtils::getfileSystemEncoding();
		...
		if(is_array($arConv))
		{
			...
			foreach($arConv as $k=>$v)
			{
					...
					|| ($v['WHEN']=='ANY'))
				{
					...
					elseif($v['THEN']=='ADD_LINK') $val = '<a class="data-ee-conversion-link" href="'.$v['TO'].'">'.$val.'</a>';
	...
	public function GetExportData($listIndex, $limit=10, $page=1, $sectionKey=1)
	{
		...
		$boolSKU = false;
		if($arCatalog = CDATAExportUtils::GetOfferIblock($iblockId, true))
		{
	...
	public function GetCachedOfferIblock($IBLOCK_ID)
	{
		...
		{
			$this->iblockoffers[$IBLOCK_ID] = CDATAExportUtils::GetOfferIblock($IBLOCK_ID, true);
		}
	...
	public function GetUserByGroups($arGroups)
	{
		...
		$xmlId = 'data_groups_'.implode('_', $arGroups);
