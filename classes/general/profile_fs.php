...
class CDATAExportProfileFS extends CDATAExportProfileAll {
	...
	function __construct($suffix='')
	{
		...
		if(!is_writable($this->pathProfiles)) $this->errors[] = sprintf(GetMessage('DATA_EE_DIR_NOT_WRITABLE'), $this->pathProfiles);
		if(!is_writable($this->tmpdir)) $this->errors[] = sprintf(GetMessage('DATA_EE_DIR_NOT_WRITABLE'), $this->tmpdir);
		if(!is_writable($this->uploadDir)) $this->errors[] = sprintf(GetMessage('DATA_EE_DIR_NOT_WRITABLE'), $this->uploadDir);
		if(file_exists($this->fileProfiles) && !is_writable($this->fileProfiles)) $this->errors[] = sprintf(GetMessage('DATA_EE_FILE_NOT_WRITABLE'), $this->fileProfiles);
	}
	...
	public function ShowProfileList($fname)
	{
		...
		?><select name="<?echo $fname;?>" id="<?echo $fname;?>" onchange="EProfile.Choose(this)"><?
			?><option value=""><?echo GetMessage("DATA_EE_NO_PROFILE"); ?></option><?
			?><option value="new" <?if($_REQUEST[$fname]=='new'){echo 'selected';}?>><?echo GetMessage("DATA_EE_NEW_PROFILE"); ?></option><?
	...
	public function GetByID($ID)
	{
		...
		if($arProfiles[$ID] && file_exists($fn))
		{
			...
			if(!is_writable($fn)) $this->errors[] = sprintf(GetMessage('DATA_EE_FILE_NOT_WRITABLE'), $fn);
		}
	...
	public function Add($name)
	{
		...
		if(strlen($name)==0)
		{
			$APPLICATION->throwException(GetMessage("DATA_EE_NOT_SET_PROFILE_NAME"));
			return false;
		}
		...
		{
			$APPLICATION->throwException(GetMessage("DATA_EE_PROFILE_NAME_EXISTS"));
			return false;
		}
	...
	{
		$arProfiles = $this->GetList();
		$newId = $this->Add($arProfiles[$ID].GetMessage("DATA_EE_PROFILE_COPY"));
